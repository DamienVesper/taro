let PixiAttributeBar = IgeEntity.extend({
    ...
    init: function (parentEntityId, attributeData, config) {
        ...
        let entity = ige.$(self._stats.parentId);
    ...
    showValueAndFadeOut: function (fadeOutDuration) {
        ...
        self.showBarTimeout = setTimeout(() => {
    ...
    fadeOut: function (duration) {
        ...
        self.updateOpacityInterval = setInterval(() => {
            self._pixiContainer.alpha -= opacityStep;
        }, 1000 / 60);

        self.destroyTimeout = setTimeout(() => {
            clearInterval(self.updateOpacityInterval);

            let parentEntity = self.getOwner();

            if (parentEntity) {
                parentEntity.attributeBars = parentEntity.attributeBars.filter((bar) => {
    ...
    renderPixiBar: function (attributeData) {
        let self = this;
        let color = PIXI.utils.string2hex(attributeData.color);
        let progressValueInPercent = (attributeData.value / attributeData.max);
        ...
        let borderRadius = self.borderRadius();
    ...
    updateBar: function (attributeData) {
        ...
        let showOnlyWhenValueChanged = attributeData.showWhen instanceof Array && attributeData.showWhen.indexOf(`valueChanges`) > -1;
