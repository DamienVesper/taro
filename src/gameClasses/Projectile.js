let Projectile = IgeEntityBox2d.extend({
    classId: `Projectile`,
    ...
    init: function (data, entityIdFromServer) {
        ...
        if (!self._stats.stateId) {
            for (let stateKey in self._stats.states) {
                let state = self._stats.states[stateKey];
        ...
        if (self._stats.states) {
            ...
            if (currentState) {
                let body = self._stats.bodies && self._stats.bodies[currentState.body] || { type: `none` };
                if (body) {
        ...
        let sourceItem = this.getSourceItem();
        ...
        if (ige.isServer) {
            ...
            if (currentState) {
                let defaultAnimation = this._stats.animations[currentState.animation];
