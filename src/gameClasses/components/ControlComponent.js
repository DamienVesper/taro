let ControlComponent = IgeEntity.extend({
    ...
    keyDown: function (device, key) {
        ...
        let player = ige.game.getPlayerByClientId(this._entity._stats.clientId);
        if (!player) {
            return;
        }
    ...
    mouseMove: function () {
        ...
        if (player) {
            if (ige.pixi && ige.pixi.viewport) {
                ...
                let mouseX = ige.client.mouseMove && ige.client.mouseMove.clientX || 0;
                let mouseY = ige.client.mouseMove && ige.client.mouseMove.clientY || 0;

                let currentMouseTransform = [
    ...
    _behaviour: function (ctx) {
        ...
        if (ige.isClient) {
            ...
            // unit move
            let unit = ige.client.selectedUnit;
            if (ige.physics && ige.game.cspEnabled && unit) {
                let x = unit._translate.x.toFixed(0);
                let y = unit._translate.y.toFixed(0);
