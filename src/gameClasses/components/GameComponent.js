var GameComponent = IgeEntity.extend({
    ...
    createPlayer: function (data, persistedData) {
        ...
        if (data.purchasables) {
            for (let i = 0; i < data.purchasables.length; i++) {
                let purchasable = data.purchasables[i];
        ...
        if (ige.isServer) {
            ...
            let isOwner = ige.server.owner == data._id;
    ...
    getAsset: function (assetType, assetId) {
        try {
            let asset = this.data[assetType][assetId];
