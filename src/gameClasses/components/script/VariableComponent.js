var VariableComponent = IgeEntity.extend({
    ...
    getRandomPositionInRegion: function (region) {
        ...
        let randomX = Math.floor(Math.random() * ((region.x + region.width) - region.x) + region.x);
        let randomY = Math.floor(Math.random() * ((region.y + region.height) - region.y) + region.y);
    ...
    isPositionInWall: function (position) {
        ...
        for (let i = 0; i < ige.map.data.layers.length; i++) {
            let layer = ige.map.data.layers[i];
            if (layer.name === `walls`) {
                wallLayer = layer;
                break;
            }
        ...
        let worldWidth = ige.map.data.width;
        let tileWidth = ige.scaleMapDetails.tileWidth || ige.game.data.map.tilewidth;
        let tileHeight = ige.scaleMapDetails.tileHeight || ige.game.data.map.tilewidth;
        ...
        let tileIndex = (tile.x) + ((tile.y) * worldWidth);
        return !!(wallLayer && wallLayer.data[tileIndex]);
    },

    isPositionInEntity: function (position) {
        let entityCategory = [`unit`, `debris`];
        let defaultArea = {
            height: 100,
            width: 100
        };

        let entities = ige.physics.getBodiesInRegion({
            x: position.x,
            y: position.y,
        ...
        let returnValue = _.some(entities, (entity) => {
            ...
            switch (text.function) {
                ...
                case `getValueOfPlayerVariable`:
                    ...
                    if (player && variableData) {
                        let playerVariable = player.variables &&
                ...
                case `getSourceUnitOfProjectile`:
                    if (entity && entity._category == `projectile`) {
                        let sourceUnitId = entity._stats.sourceUnitId;
                ...
                case `getPlayerCount`:
                    returnValue = ige.$$(`player`).filter((player) => {
                ...
                case `getRandomPlayablePositionInRegion`:
                    ...
                    if (region) {
                        for (var i = 0; i < attempts; i++) {
                            ...
                            let isPlayablePosition = !self.isPositionInEntity(position) &&
                ...
                case `getEntityPosition`:
                    ...
                    if (entity) {
                        if (entity._category === `item` && entity._stats && entity._stats.currentBody && entity._stats.currentBody.type === `spriteOnly`) {
                            let ownerUnit = entity.getOwnerUnit();
                            let unitPosition = _.cloneDeep(ownerUnit._translate);
                ...
                case `getRandomItemTypeFromItemTypeGroup`:
                    if (text.itemTypeGroup) {
                        ...
                        if (variableObj) {
                            let itemTypes = _.map(variableObj, (itemTypeInfo, itemType) => {
                                return {
                                    itemType,
                ...
                case `getRandomUnitTypeFromUnitTypeGroup`:
                    if (text.unitTypeGroup) {
                        ...
                        if (variableObj) {
                            let unitTypes = _.map(variableObj, (unitTypeInfo, unitType) => {
                                return {
                                    unitType,
                ...
                case `getItemParticle`:
                    ...
                    if (entity && entity._category == `item` && particleTypeId) {
                        if (entity._stats.particles) {
                            let particleType = entity._stats.particles[particleTypeId];
                ...
                case `allUnitsOwnedByPlayer`:
                    ...
                    if (player) {
                        ...
                        for (var i = 0; i < player._stats.unitIds.length; i++) {
                            let unitId = player._stats.unitIds[i];
                ...
                case `allUnitsInRegion`:
                    ...
                    if (region) {
                        let regionBounds = region._stats ? region._stats.default : region;
                ...
                case `humanPlayers`:
                    returnValue = ige.$$(`player`).filter((player) => { return player._stats.controlledBy == `human`; });
                    break;
                ...
                case `getValueOfEntityVariable`:
                    ...
                    if (entity && variableData) {
                        let entityVariable = entity.variables &&
    ...
    calculate: function (items, vars) {
        ...
        if ((items && items.constructor != Array) || typeof items === `number`) {
            let solution = this.getValue(items, vars);
            return parseFloat(solution);
        }

        let op = items[0];
        let left = this.getValue(items[1], vars);
        let right = this.getValue(items[2], vars);
    ...
    getAllVariables: function (selectedTypes) {
        ...
        for (let variableName in ige.game.data.variables) {
            let variable = ige.game.data.variables[variableName];
            ...
            if (!selectedTypes || selectedTypes.includes(variable.dataType)) {
                ...
                if (variable.value == undefined && variable.default != undefined) {
                    if (variable.dataType == `player` || variable.dataType == `unit`) {
                        ...
                        let region = ige.regionManager.getRegionById(variableName);
    ...
    updateDevConsole: function (data) {
        ...
        if (ige.isServer && (ige.server.developerClientId || process.env.ENV === `standalone` || process.env.ENV == `standalone-remote`)) {
            ...
            let totalAttrsCount = 0;
            for (variableName in data) {
                if (variableName !== `status`) {
                    let div = $(`#variables-div div.col-sm-12[name='${variableName}']`);
                    let newValue = data[variableName];
                    if (div.length) {
