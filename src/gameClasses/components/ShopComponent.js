let ShopComponent = IgeEntity.extend({
    ...
    init: function (entity, options) {
        ...
        if (ige.isClient) {
            ...
            $(`.open-modd-shop-button`).on(`click`, () => {
            ...
            $(`.open-coin-shop-button`).show().on(`click`, () => {
                self.openCoinShop();
            });
            ...
            $(`.item-shop-navbar .nav-item`).on(`click`, function () {
                if (!$(this.firstElementChild).hasClass(`active`)) {
                    ...
                    let selected = $(this).find(`.nav-link`).attr(`name`);
            ...
            $(`#mod-shop-pagination`).on(`click`, `.skin-pagination`, function () {
                let itemDom = $(this);
                let buttonPressed = itemDom[0].dataset.text;
                let totalPages = Math.ceil(self.skinItems.length / self.perPageItems);
            ...
            $(document).on(`click`, `.btn-purchase-item`, function () {
                ...
                // }
                // else {
                let isItemRequirementSetisfied = $(this).attr(`requirementsSatisfied`) == `true`;
                let isItemAffordable = $(this).attr(`isItemAffordable`) == `true`;
            ...
            $(`#modd-item-shop-modal`).on(`hidden.bs.modal`, () => {
                $(`.popover`).remove();
            });
            ...
            $(document).on(`click`, `.btn-purchase-purchasable`, function () {
                ...
                let itemDom = $(this);
                let name = itemDom[0].dataset.purchasable;
                let price = isNaN(parseFloat(itemDom[0].dataset.price)) ? itemDom[0].dataset.price : parseFloat(itemDom[0].dataset.price);
                let isUnauthenticated = itemDom[0].dataset.unauthenticated;
                ...
                hasSharedDefer.promise()
                    .then((hasShared) => {
                        if (hasShared) {
                            let itemId = itemDom.attr(`id`);
                            let gameData = ige.game.data.defaultData;

                            if (price === `facebook` || price === `twitter`) {
                                let item = { value: gameData._id, type: `game` };
                                let from = `shopModal`;
                                ...
                                if (price === `facebook`) {
                                    ...
                                    shareOnFacebook(item, from, config, (response) => {
                                        if (response) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (response.status == `success`) {
                            ...
                            let equipedPurchasable = response.message;
                            // ige.client.myPlayer._stats.purchasables.push(equipedPurchasable);
                            let myUnit = ige.$(ige.client.myPlayer._stats.selectedUnitId);
            ...
            $(document).on(`click`, `button.btn-unequip`, function () {
                let button = $(this);
                let unEquipedId = button.attr(`id`);
                $.ajax({
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == `success`) {
                    ...
                    let userPurchasedItemIds = userPurchases.reduce((partial, purchase) => {
                        partial[purchase._id] = true;
                        return partial;
                    }, {});

                    let purchasableItems = typeof purchasables.filter === `function` && purchasables.filter((purchasable) => {
                    ...
                    purchasableItems.forEach((purchasable, index) => {
                        let skin = $(
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (response.status == `success`) {
                    ...
                    let details = $(`.btn-purchase-purchasable#${itemId}`);
                    let purchasableInfo = details.find(`.purchasable-details`);
                    ...
                    if (!response.message.includes(`No matching document found`)) {
                        let error = `<div class="alert alert-danger text-center">${response.message}</div>`;
    ...
    hideEmptyTabs: function () {
        ...
        if (!ige.game.data.unitTypes) {
            ...
            for (unitId in ige.game.data.unitTypes) {
                let unit = ige.game.data.unitTypes[unitId];
        ...
        if (!ige.game.data.itemTypes) {
            ...
            for (itemId in ige.game.data.itemTypes) {
                let item = ige.game.data.itemTypes[itemId];
                    ...
                    if (data.message) {
                        if (!data.message.unitCount || data.message.unitCount.length === 0) {
                            ...
                            for (var i in ige.game.data.unitTypes) {
                                var total = data.message.unitCount.filter((unit, key) => {
                        ...
                        if (!data.message.itemCount || data.message.itemCount.length === 0) {
                            ...
                            for (var i in ige.game.data.itemTypes) {
                                var total = data.message.itemCount.filter((item, key) => {
    ...
    buttonForSocialShare: function (item, unauthenticated) {
        let btnHtml = ``;
        let sharedOn = item.sharedOn || {};
        let sharedOnFacebook = sharedOn.facebook ? `disabled` : ``;
        let sharedOnTwitter = sharedOn.twitter ? `disabled` : ``;
        let gameId = ige.game.data.defaultData._id;

        let textToTweet = `Need more players!! Join me in ${ige.game.data.defaultData.title} (${location.href})`;
        let hashTags = [`games`, `moddio`];
        let mentions = [`moddio`];
        let twitterUrl = `https://twitter.com/intent/tweet?url=share.url&text=${textToTweet}&hashtags=${hashTags.join(`,`)}&via=${mentions.join(`,`)}`;
    ...
    updateModdShop: function (tabSelected) {
        ...
        if (self.shopType == `unitSkins`) {
            ...
            let unitKeys = Object.keys(ige.game.data.unitTypes);
            unitKeys = unitKeys.sort();

            // generating li column for unit type selection
            for (let p = 0; p < unitKeys.length; p++) {
                var key = unitKeys[p];
                $(`#modd-shop-modal .shop-items`).html(``);
        ...
        if (self.shopType == `unitSkins` || self.shopType == `itemSkins`) // skins
        {
            $.ajax({
                ...
                type: `GET`,
                success: function (data) {
                    let items = JSON.parse(data).message;
                    let groupedItems = {
                        purchased: [],
                        notPurchased: []
                    };
    ...
    updateSkinList: function (itemId) {
        debugger;
        let skinItemActionButton = $(`#skin-list-${itemId} .action-button-container`);
    ...
    isItemRequirementSetisfied: function (req, priceAttr) {
        let ownerPlayer = ige.client.myPlayer;
    ...
    getItemPopoverHtml: function (item, shopItem) {
        ...
        let html = ``;
        let ownerPlayer = ige.client.myPlayer;
        let ownerUnit = ige.$(ownerPlayer._stats.selectedUnitId);
        if (item.description) {
        ...
        if (shopItem && typeof shopItem.requirement === `object`) {
            ...
            for (var priceAttr in shopItem.requirement.playerAttributes) {
                let req = shopItem.requirement.playerAttributes[priceAttr];
        ...
        if (shopItem && typeof shopItem.price === `object`) {
            ...
            for (var priceAttr in shopItem.price.playerAttributes) {
                let playerAttrValue = ownerPlayer._stats.attributes[priceAttr] && ownerPlayer._stats.attributes[priceAttr].value || 0;
    ...
    openItemShop: function (type, selectedTab) {
        ...
        let shopItemsKeys = ige.game.data.shops[self.currentType] ? Object.keys(ige.game.data.shops[self.currentType].itemTypes || {}) : [];
        shopItemsKeys = shopItemsKeys.sort();
        let shopUnitsKeys = ige.game.data.shops[self.currentType] ? Object.keys(ige.game.data.shops[self.currentType].unitTypes || {}) : [];
        shopUnitsKeys = shopUnitsKeys.sort();
        let shopItems = ige.game.data.shops[self.currentType] ? _.cloneDeep(ige.game.data.shops[self.currentType].itemTypes) : [];
        let shopUnits = ige.game.data.shops[self.currentType] ? ige.game.data.shops[self.currentType].unitTypes : [];
        let isDismissible = ige.game.data.shops[self.currentType] && ige.game.data.shops[self.currentType].dismissible != undefined ? ige.game.data.shops[self.currentType].dismissible : true;
        let shopItemsKeysUsingCoins = [];
        ...
        let isAdBlockEnabled = ownerPlayer._stats.isAdBlockEnabled;
        let ownerUnit = ige.$(ownerPlayer._stats.selectedUnitId);
        ...
        if (selectedTab === `items`) {
            ...
            for (var i = 0; i < shopItemsKeys.length; i++) {
                let item = ige.game.getAsset(`itemTypes`, shopItemsKeys[i]);
                let shopItem = shopItems[shopItemsKeys[i]];
                ...
                if (item && shopItem && shopItem.isPurchasable) {
                    ...
                    let bgColor = requirementsSatisfied && isItemAffordable ? `bg-light-green` : `bg-light-red`;
                    let itemImage = $(`<div/>`, {
                        id: shopItemsKeys[i],
                    ...
                    if (
                        ...
                        let img = $(`<div/>`).html(`img_index_${imgArray.length}`);
                        imgArray.push({
                            wrapper: img,
                            value: `<img src='${item.inventoryImage || item.cellSheet.url}' style='width: auto; height: auto; max-width: 55px; max-height: 55px'>`
                        });

                        let itemName = `<div class='mx-2 mt-2 mb-0 no-selection' style='line-height:0.7  !important; overflow-wrap: break-word;'><small>`;
                        itemName += item.name;
                        itemName += `</small></div>`;
            ...
            for (var i = 0; i < shopUnitsKeys.length; i++) {
                let unit = ige.game.getAsset(`unitTypes`, shopUnitsKeys[i]);
                let shopUnit = shopUnits[shopUnitsKeys[i]];
                ...
                if (shopUnit && shopUnit.isPurchasable) {
                    ...
                    let btnLabel = self.shopBtnLabel(shopUnit, playerTypeAttribute);
    ...
    shopBtnLabel: function (type, playerTypeAttribute) {
        ...
        let coins = 0;
        let itemsRequired = [];
        let attributeTypes = ige.game.data.attributeTypes;
        ...
        priceKey.forEach((key) => {
            let price = type.price.playerAttributes[key];
        ...
        let requiredItemTypesKeys = Object.keys(itemsRequired || {});
        for (let i = 0; i < requiredItemTypesKeys.length; i++) {
            let itemKey = requiredItemTypesKeys[i];
            let requiredQty = itemsRequired[itemKey];
            let item = ige.game.getAsset(`itemTypes`, itemKey);
            if (btnLabel == `free`) {
                btnLabel = ``;
            }
            if (btnLabel) {
    ...
    paginationForSkins: function () {
        ...
        let totalPages = Math.ceil(self.skinItems.length / self.perPageItems);
        let maxPageNumber = Math.min(11, totalPages);
        if (ige.mobileControls && ige.mobileControls.isMobile) {
            maxPageNumber = Math.min(3, totalPages);
        }
        let currentPage = self.currentPagination - 1;
        let skip = currentPage * self.perPageItems;
        let items = _.slice(self.skinItems, skip, skip + self.perPageItems);
        ...
        for (let i = Math.max(1, currentPage); i < Math.min(totalPages + 1, currentPage + maxPageNumber + 1); i++) {
    ...
    addItem: function (data) {
        ...
        if (ige.isServer) {
            ...
            var self = this;
            let modalBody = $(`#shop-modal .modal-body`);
            $(`.popover`).remove();

            // console.log("adding item", data)
            let itemDiv = ige.itemUi.getItemSlotDiv(data, {
                popover: `bottom`,
                isDraggable: false,
                isPurchasable: true
            });
    ...
    removeOldestItem: function () {
        let self = this;
        let item = self.inventory.shift(); // remove the oldest item
    ...
    getItemById: function (itemTypeId) {
        let itemData = ige.game.getAsset(`itemTypes`, itemTypeId);
        if (itemData) {
    ...
    getUnitById: function (unitId) {
        let unitData = ige.game.getAsset(`unitTypes`, unitId);
        if (unitData) {
