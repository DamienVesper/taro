let TweenComponent = IgeEntity.extend({
    ...
    */
    ...
    update: function () {
        ...
        if (this.keyFrames.length > 0) {
            let nextFrameEndsAt = this.startTime + this.nextFrame[0];
            if (ige._currentTime < nextFrameEndsAt) {
                let x = this.lastFrame[1][0];
                let y = this.lastFrame[1][1];
                let rotate = this.lastFrame[1][2];
                let targetX = this.nextFrame[1][0];
                let targetY = this.nextFrame[1][1];
                let targetRotate = this.nextFrame[1][2];
                ...
                let interpolatedX = this._entity.interpolateValue(x, targetX, this.startTime, ige._currentTime, nextFrameEndsAt);
                let interpolatedY = this._entity.interpolateValue(y, targetY, this.startTime, ige._currentTime, nextFrameEndsAt);
                let interpolatedRotate = this.interpolateValue(rotate, targetRotate, this.startTime, ige._currentTime, nextFrameEndsAt);
                ...
                if (this._entity._category == `item` && this._entity._stats.currentBody) {
                    if (this._entity._stats.currentBody.jointType == `weldJoint`) {
                        let ownerUnit = this._entity.getOwnerUnit();
                        if (ownerUnit) {
                    ...
                    if (this.offset.rotate != 0) {
                        ...
                        let before = this._entity.getAnchoredOffset(rotate);
                        let after = this._entity.getAnchoredOffset(rotate + interpolatedRotate);
