let MapComponent = IgeEntity.extend({
    ...
    init: function (entity, options) {
        let self = this;
        self.minimapLayers = [];
        self.debrisProperty = [`x`, `y`, `height`, `width`, `rotation`, `visible`, `type`, `id`, `name`, `gid`, `type`, `density`, `friction`, `restitution`];
        self.layersZIndex = {
            floor: 0,
            floor2: 1,
            walls: 3,
        ...
        if (ige.isClient) {
            ...
            if (mode === `sandbox`) {
                $(`#debris-form`).on(`submit`, (e) => {
                    e.preventDefault();
                    self.onSubmit();
                });

                $(`#debris-form`).on(`keypress`, (e) => {
                    if (e.charCode === 13) {
                        e.preventDefault();
                        self.onSubmit();
                    }
    ...
    load: function (data) {
        ...
        if (ige.isServer) {
            ige.addComponent(IgeTiledComponent)
                .tiled.loadJson(self.data, (layerArray, layersById) => {
                    ...
                    let mapArray = layersById.floor.map._gameData;
                    ...
                    if (ige.isClient) {
                        $.when(ige.client.igeEngineStarted).done(() => {
                            ...
                            if (mode === `sandbox`) {
                                ...
                                let region = new RegionUi({ height: mapHeight, width: mapWidth });
                                region.depth(3)
                                    .layer(3)
            ...
            ige.addComponent(IgeTiledComponent)
                .tiled.loadJson(self.data, (IgeLayerArray, IgeLayersById) => {
                    ige.addComponent(IgePixiMap)
                        .pixiMap.loadJson(self.data, (layerArray, layersById) => {
                            self.minimapLayers = _.cloneDeep(layerArray);
                            ...
                            // object. Let's give an example:
                            if (mode === `sandbox`) {
                                let mapHeight = ige.game.data.map.height * ige.game.data.map.tileheight;
                                let mapWidth = ige.game.data.map.width * ige.game.data.map.tilewidth;
                                let region = new RegionUi({ height: mapHeight, width: mapWidth });
                                region.depth(3)
                                    .layer(3)
                                    .drawBoundsData(false)
    ...
    createRegions: function () {
        let regions = {};
        for (let i in ige.game.data.variables) {
            let variable = ige.game.data.variables[i];
    ...
    createDebris: function (debrisLayer) {
        ...
        for (var i = 0; i < self.data.tilesets.length; i++) {
            let tileset = self.data.tilesets[i];
            let imageUrl = tileset.image;
                ...
                if (ige.game.data.isDeveloper) {
                    debris.drawMouse(true)
                        .mouseDown(function (event, evc) {
                            let isMiniMapClicked = ige.mapEditor.checkIfClickedMiniMap(event.pageX, event.pageY);
    ...
    onSubmit: function () {
        let self = this;
        let updatedDebris = {};
        for (let i = 0; i < ige.map.debrisProperty.length; i++) {
            let value = ``;
                ...
                if (data.status === `success`) {
                    ...
                    if (updatedDebris.deleteId) {
                        ...
                        if (debris && debris._stats) {
                            for (let i = 0; i < ige.map.debrisProperty.length; i++) {
                                let property = ige.map.debrisProperty[i];
