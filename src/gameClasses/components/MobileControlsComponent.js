 ...
 */
let MobileControlsComponent = IgeEntity.extend({
    ...
    init: function (entity) {
        ...
        let canvas = document.getElementById(`igeFrontBuffer`);
        this.canvas = {
        ...
        $(window).on(`orientationchange load resize`, () => {
            ...
            */
            ...
            if (ige.mobileControls) {
                ...
                if (ige.client && ige.client.myPlayer && ige.network.id() == ige.client.myPlayer._stats.clientId) {
                    let unit = ige.$(ige.client.myPlayer._stats.selectedUnitId);
                    if (unit && unit._stats.controls) {
                        let unitAbilities = unit._stats.controls.abilities;
                        if (unitAbilities) {
    ...
    isSingleGameMode: function () {
        let s = this.getParameterByName(`s`);
    ...
    clearControls: function (newAbilities) {
        ...
        for (ix in this.controls) {
            ...
            if (newAbilities && !newAbilities[uppercaseKey]) {
                let button = self.controls[uppercaseKey];
    ...
    configure: function (abilities) {
        ...
        Object.keys(abilities).forEach((key, index) => {
            ...
            if (ability.mobilePosition && !self.controls[key]) {
                ...
                // rescale to 960x540
                let x = ability.mobilePosition.x * 2;
                let y = ability.mobilePosition.y * 2;
    ...
    upPressed: function () {
        ...
        let unit = ige.client.myPlayer.getSelectedUnit();
        unit.ability.moveUp();
        if (ige.isClient) {
    ...
    addControl: function (key, x, y, w, h, ability) {
        ...
        let isStick = (key == `movementWheel` || key == `lookWheel` || key == `lookAndFireWheel`);
        if (isStick) {
                ...
                let moveStick = new Joystick({
                    ...
                    onChange: (data) => {
                        if (ige.client.myPlayer) {
                            ...
                            let compassAngle = (360 - (data.angle - 90)) % 360;

                            let tolerance = 12;

                            let isUp = (compassAngle <= 90 - tolerance) || (compassAngle >= 270 + tolerance);
                            let isDown = (compassAngle >= 90 + tolerance) && (compassAngle <= 270 - tolerance);
                            let isLeft = (compassAngle <= 360 - tolerance) && (compassAngle >= 180 + tolerance);
                            let isRight = (compassAngle >= tolerance) && (compassAngle <= 180 - tolerance);
                ...
                let lookStick = new Joystick({
                    ...
                    onChange: (data) => {
                        if (ige.client.myPlayer) {
                            // Endel's joystick angles are in "Maths style" (zero degrees is EAST and positive anticlockwise)
                            // Convert into compass style angle (zero degrees NORTH and positive clockwise)
                            let compassAngle = -(data.angle - 90);

                            // simulate mouse movement 10 units away from player (scaled by joystick "power") character but at the angle
                            let unitTranslate = ige.client.myPlayer.getSelectedUnit()._translate;
                            let mx = unitTranslate.x + Math.sin(compassAngle / 360 * 2 * Math.PI) * 10 * data.power;
                            let my = unitTranslate.y - Math.cos(compassAngle / 360 * 2 * Math.PI) * 10 * data.power;
            ...
            let newButton = new PIXI.Sprite.from(`https://cache.modd.io/asset/spriteImage/1549614640644_button1.png?version=123`, { crossOrigin: true });
            ...
            if (iconUrl) {
                let icon = new PIXI.Sprite.from(iconUrl, { crossOrigin: true });
                ige.pixi.mobileControls.addChild(icon);
                icon.scale.set(0.5);
                icon.anchor.set(0.5);
                icon.position.set(x + (w / 2), y + (h / 2));
            } else {
                let label = new PIXI.Text(text, { fontFamily: `Arial`, fontSize: 24, fill: 0xffffff, align: `center` });
