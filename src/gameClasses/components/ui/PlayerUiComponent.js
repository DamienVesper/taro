let PlayerUiComponent = IgeEntity.extend({
    ...
    setupListeners: function () {
        ...
        $(`#player-input-modal`).on(`hidden.bs.modal`, () => {
        ...
        $(`#custom-modal`).on(`hidden.bs.modal`, () => {
            $(`#custom-modal`).removeClass(`d-flex`);
        });

        $(`#custom-modal`).on(`shown.bs.modal`, () => {
            $(`#custom-modal-cancel`).focus();
        });

        $(`#player-input-modal`).on(`shown.bs.modal`, () => {
        ...
        $(`button#player-input-cancel`).on(`click`, () => {
            self.pressedButton = false;
    ...
    updatePlayerAttributesDiv: function (attributes) {
        ...
        for (let attrKey in attributes) {
            ...
            if (attr) {
                ...
                let attributeType = attributeTypes[attrKey];
                let name = attributeType ? attributeType.name : attr.name;
                ...
                if (attr.value % 1 === 0) {
                    ...
                    if (attr.decimalPlaces != undefined && attr.decimalPlaces != null) {
                        let decimalPlace = parseInt(attr.decimalPlaces);
                        if (decimalPlace != NaN) {
                ...
                let value = attr.value && attr.value.toLocaleString(`en-US`) || 0;
    ...
    updatePlayerCoin: function (newValue) {
        let coin = parseInt($(`.player-coins`).html());
        if (coin != NaN) {
    ...
    openWebsite: function (config) {
        ...
        config.isDismissible = config.isDismissible === undefined ? true : !!(config.isDismissible);
        let newWin = window.open(config.url);
    ...
    openDialogueModal: function (dialogueId, extraData) {
        ...
        function getDialogueInstance (dialogue) {
            let playerName = extraData && extraData.playerName;
            ...
            let variables = dialogue.message.match(new RegExp(`\\$.+?\\$`, `g`));

            if (variables && variables.length) {
                variables.forEach((variable) => {
                    // variable are in format $xxx$ so splitting it by $ will give ['', 'xxx', '']
                    let variableName = variable.split(`$`)[1];

                    if (extraData.variables.hasOwnProperty(variableName)) {
                        let variableValue = extraData.variables[variableName];
        ...
        function showOptions () {
            ...
            for (let key in dialogue.options) {
                let optionObject = dialogue.options[key];
                let button = $(`<button/>`, {
                    id: key,
    ...
    submitDialogueModal: function (dialogueId, optionId) {
        let self = this;
        let willOpenNewDialogue = false;
        let dialogue = ige.game.data.dialogues[dialogueId];
