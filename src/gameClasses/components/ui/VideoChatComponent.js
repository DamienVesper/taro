let VideoChatComponent = IgeEntity.extend({
    ...
    updatePlayerGroups: function (self) {
        let players = ige.$$(`player`).filter((player) => { return player._stats.controlledBy == `human`; });
        // var players = ige.$$('player')

        // update centoid of the groups
        for (groupId in self.groups) {
            var group = self.groups[groupId];
            let polygons = self.getPolygons(group.playerIds);
                ...
                if (unit) {
                    if (player.vcGroupId) {
                        ...
                        for (let j = 0; j < players.length; j++) {
                            ...
                            if (playerB) {
                                let playerBId = playerB.id();
                                if (playerId != playerBId) {
                                    let unitB = playerB.getSelectedUnit();
    ...
    createGroup: function (playerIds) {
        ...
        let polygons = self.getPolygons(self.groups[groupId].playerIds);
    ...
    destroyGroup: function (id) {
        for (groupId in this.groups) {
            if (groupId == id) {
                ...
                let playerIds = this.groups[id].playerIds;
                if (playerIds) {
                    for (i = 0; i < playerIds.length; i++) {
                        let playerId = playerIds[i];
    ...
    removePlayerFromGroup: function (playerId) {
        ...
        if (player) {
            if (player.vcGroupId) {
                let playerIds = this.groups[player.vcGroupId].playerIds;
                if (playerIds) {
                    for (i = 0; i < playerIds.length; i++) {
