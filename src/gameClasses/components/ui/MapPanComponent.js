 ...
 */
let MapPanComponent = IgeEventingClass.extend({
    ...
    enabled: function (val) {
        ...
        if (val !== undefined) {
            ...
            if (this._enabled) {
                ...
                this._entity.mouseDown((event) => { if (event.which === 3) self._mouseDown(event); });
                this._entity.mouseMove((event) => { self._mouseMove(event); });
                this._entity.mouseUp((event) => { self._mouseUp(event); });
                this._entity.mouseWheel((event) => { self._mouseWheel(event); });
            } else {
    ...
    _mouseMove: function (event) {
        let vpId = event && event.igeViewport && event.igeViewport.id();
        if (vpId === `minimapVp`) {
            vpId = `vp1`;
        }
        if (vpId)
    ...
    _mouseWheel: function (event) {
        if (this._zoomEnabled) {
            ...
            let scale = this.zoomStartThreshold / 100;
