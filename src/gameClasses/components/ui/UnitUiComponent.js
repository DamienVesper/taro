let UnitUiComponent = IgeEntity.extend({
    ...
    updateAllAttributeBars: function () {
        ...
        let attributes = self._entity._stats.attributes; // get unit's attribute types
        ...
        if (ige.isClient && ige.network.id() == self._entity._stats.clientId) {
            ...
            let belongsToSelectedUnit = ownerPlayer && ownerPlayer._stats && ownerPlayer._stats.selectedUnitId === self._entity.id();

            if (belongsToSelectedUnit) {
                $(`#attribute-bars`).html(``);
            }

            let isAttributeBarPresent = false;
            for (let attributeTypeId in attributes) {
                ...
                if (belongsToSelectedUnit) {
                    ...
                    if (
                        ...
                        let width = (attribute.value / attribute.max) * 100;
    ...
    updateAttributeBar: function (attr) {
        ...
        let name = attributeTypes[attr.type] ? attributeTypes[attr.type].name : attr.name;
        ...
        if (
            ...
            let widthInPercent = (attr.value / attr.max) * 100;
