let SoundComponent = IgeEntity.extend({
    ...
    preLoadSound: function () {
        let self = this;
        for (let soundKey in ige.game.data.sound) {
            let sound = ige.game.data.sound[soundKey];
    ...
    preLoadMusic: function () {
        let self = this;
        for (let musicKey in ige.game.data.music) {
            let music = ige.game.data.music[musicKey];
    ...
    getVolume: function (position, volume = 0) {
        let settingsVolume = parseFloat(localStorage.getItem(`sound-volume`));
        settingsVolume = isNaN(settingsVolume) ? 1 : settingsVolume / 100;

        let distanceSoundShouldHeard = 500;
        ...
        let vpBound = ige.pixi.viewport.getVisibleBounds();
        let myPosition = { x: vpBound.x + vpBound.width / 2, y: vpBound.y + vpBound.height / 2 };
        let xDistance = position.x - myPosition.x;
        let yDistance = position.y - myPosition.y;
    ...
    playSound: function (sound, position, key, shouldRepeat = false) {
        ...
        if (ige.isClient) {
            ...
            if (soundSetting == `on`) {
                ...
                let volume = position === null ? sound.volume / 100 : 0;
                if (position) {
                ...
                if (sound && sound.file) {
                    if (self.preLoadedSounds[key] && self.preLoadedSounds[key].src == sound.file) {
                        ...
                        self.preLoadedSounds[key].play().catch((e) => {
                            console.log(e);
                        });
                        if (shouldRepeat) {
    ...
    playMusic: function (music, startAt, shouldRepeat, key) {
        ...
        if (ige.isClient) {
            let musicSetting = localStorage.getItem(`music`);
            let settingsVolume = parseFloat(localStorage.getItem(`music-volume`));
            settingsVolume = isNaN(settingsVolume) ? 1 : settingsVolume / 100;

            let volume = music.volume === undefined ? 1 : Math.min(music.volume / 100, 1);
    ...
    startMusic: function () {
        if (ige.isClient) {
            if (this.musicCurrentlyPlaying) {
                this.musicCurrentlyPlaying.play().catch((e) => {
                    console.log(e);
                });
    ...
    updateMusicVolume: function (volume) {
        if (ige.isClient) {
            if (this.musicCurrentlyPlaying) {
                let originalVolume = this.musicCurrentlyPlaying.originalVolume;
