var AbilityComponent = IgeEntity.extend({
    ...
    startUsingItem: function () {
        let item = this._entity.getCurrentItem();
        if (item) {
            item.startUsing();
        }
    },
    ...
    pickupItem: function () {
        ...
        if (ige.isServer) {
            ...
            let entities = ige.physics.getBodiesInRegion(region).filter(({ _category }) => _category === `item`);
    ...
    dropItem: function () {
        ...
        if (self._entity && !isNaN(self._entity._stats.currentItemIndex)) {
            let item = self._entity.dropItem(self._entity._stats.currentItemIndex);
            if (item) {
    ...
    cast: function (handle) {
        ...
        let player = self._entity.getOwner();
