let AIComponent = IgeEntity.extend({
    classId: `AIComponent`,
    componentId: `ai`,
    ...
    registerAttack: function (unit) {
        ...
        if (unit) {
            let ownerPlayerOfTargetUnit = ige.$(unit._stats.ownerId);
            if (ownerPlayer && ownerPlayer.isHostileTo(ownerPlayerOfTargetUnit)) {
                ...
                let targetUnit = this.getTargetUnit();
    ...
    announceDeath: function () {
        let unitIds = this.unitsTargetingMe;
        for (let i = 0; i < unitIds.length; i++) {
            let attackingUnit = ige.$(unitIds[i]);
    ...
    getDistanceToTarget: function () {
        ...
        if (targetUnit) {
            ...
            let targetPosition = this.getTargetPosition();
            if (targetPosition) {
                let a = unit._translate.x - targetPosition.x;
                let b = unit._translate.y - targetPosition.y;
    ...
    getDistanceToUnit: function (targetUnit) {
        let myUnit = this._entity;
        if (targetUnit) {
            let a = Math.abs(myUnit._translate.x - targetUnit._translate.x);
            let b = Math.abs(myUnit._translate.y - targetUnit._translate.y);
            var distanceFromCenter = Math.sqrt(a * a + b * b);
        }

        let myUnitReach = myUnit.height() / 2;
        let targetUnitRadius = targetUnit.width() / 2;
    ...
    goIdle: function () {
        let unit = this._entity;
        let currentItem = unit.getCurrentItem();
        if (currentItem) {
    ...
    update: function () {
        ...
        let targetPosition = self.getTargetPosition();
