var Item = IgeEntityBox2d.extend({
    ...
    mount: function (obj) {
        let state = this._stats.states && this._stats.states[this._stats.stateId] || {};
        ...
        if (obj && obj._category == `unit`) {
            if (body && body.type != `none`) {
                ...
                if (ige.isClient) {
                    ...
                    let unitAnchorX = body.unitAnchor.x;
                    let unitAnchorY = body.unitAnchor.y;
    ...
    updateTexture: function () {
        ...
        let ownerUnit = self.getOwnerUnit();
        ...
        if (ownerUnit) {
            ...
            let hasBody = self._stats.currentBody && self._stats.currentBody.type !== `none`;
                                ...
                                if (this._stats.bulletType == `raycast`) {
                                    ...
                                    let raycastMultipleCallback = function () {
                                        ...
                                        def.ReportFixture = function (fixture, point, normal, fraction) {
                                            let fixtureList = fixture.m_body.m_fixtureList;
                                            let entity = fixtureList && fixtureList.igeId && ige.$(fixtureList.igeId);
                                            if (entity) {
    ...
    canAffordItemCost: function () {
        ...
        if (player && owner && ability.cost) {
            ...
            let unitAttributeChanged = false;
            let playerAttributeChanged = false;
    ...
    refillAmmo: function () {
        let itemData = ige.game.getAsset(`itemTypes`, this._stats.itemTypeId);
    ...
    remove: function () {
        ...
        let ownerUnit = ige.$(this._stats.ownerUnitId);
        if (ownerUnit) {
    ...
    _behaviour: function (ctx) {
        ...
        if (ownerUnit && this._stats.stateId != `dropped`) {
            ...
            let x = ownerUnit._translate.x + self.anchoredOffset.x;
            let y = ownerUnit._translate.y + self.anchoredOffset.y;
