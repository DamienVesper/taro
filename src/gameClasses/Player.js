var Player = IgeEntity.extend({
    ...
    init: function (data, entityIdFromServer) {
        ...
        if (ige.isServer) {
            ...
            if (self._stats.clientId == ige.network.id()) {
                ...
                // mouse move listener
                ige.client.vp1.mouseMove((event, control) => {
    ...
    joinGame: function () {
        ...
        if (self._stats.playerJoined != true) {
            ...
            let clientId = self._stats.clientId;
            let client = ige.server.clients[clientId];
            let receivedJoinGame = client.receivedJoinGame;
            let processedJoinGame = Date.now() - receivedJoinGame;
            let dataLoadTime = self._stats.totalTime;
            client.lastEventAt = Date.now();

            let playerJoinStreamData = [
                { streamedOn: Date.now() },
                { playerJoined: true },
    ...
    disownUnit: function (unit) {
        let index = this._stats.unitIds.indexOf(unit.id());
        if (index !== -1) {
    ...
    selectFirstAvailableUnit: function () {
        if (this._stats.unitIds) {
            let unit = ige.$(this._stats.unitIds[0]);
            if (unit) {
    ...
    castAbility: function (command) {
        let self = this;
        let units = ige.game.getUnitsByClientId(this._stats.clientId);
        for (i in units) {
            let unit = units[i];
            if (unit) {
                let unitType = ige.game.getAsset(`unitTypes`, unit._stats.type);
                if (unitType && unitType.abilities) {
                    let ability = unitType.abilities[command];
                    if (ability) {
    ...
    updatePlayerType: function (data) {
        ...
        if (self._stats.attributes != undefined) {
            let oldAttributes = JSON.parse(JSON.stringify(self._stats.attributes));
        ...
        if (ige.isClient) {
            let isMyPlayerUpdated = self._stats.clientId == ige.network.id();
    ...
    isHostileTo: function (player) {
        ...
        let myPlayerType = ige.game.getAsset(`playerTypes`, this._stats.playerTypeId);
    ...
    updateVisibility: function (playerId) {
        ...
        ige.$$(`item`)
            .forEach((item) => {
                ...
                let ownerPlayer = ownerUnit && ownerUnit.getOwner();
                let belongsToCurrentPlayer = ownerPlayer && ownerPlayer.id() === playerId;
    ...
    streamUpdateData: function (queuedData) {
        let self = this;
        let oldStats = JSON.parse(JSON.stringify(self._stats));
        ...
        for (let i = 0; i < queuedData.length; i++) {
            ...
            for (attrName in data) {
                ...
                if (attrName === `playerTypeId`) {
                    let playerTypeData = ige.game.getAsset(`playerTypes`, newValue);
                    if (playerTypeData) {
                        ...
                        if (ige.isClient) {
                            ...
                            self.redrawUnits((unit) => {
                                ...
                                let ownerPlayer = unit.getOwner();
                                let unitBelongsToUpdatingPlayer = ownerPlayer && ownerPlayer.id() === self.id();
                ...
                if (ige.isServer) {
                    if (attrName === `name` && oldStats.name !== newValue) {
                        // update all units
                        self._stats.unitIds.forEach((unitId) => {
                            let unit = ige.$(unitId);
                ...
                if (ige.isClient) {
                    ...
                    if (self._stats.clientId == ige.network.id()) {
                        ...
                        if (attrName === `playerJoined`) {
                            ...
                            let streamingDiff = `${Date.now() - data.streamedOn}ms`;
    ...
    updatePlayerHighscore: function () {
        let self = this;
        let scoreId = ige.game.data.settings.scoreAttributeId;
        try {
            ...
            if (scoreId && self._stats && self._stats.attributes && self._stats.attributes[scoreId] && (self._stats.highscore < self._stats.newHighscore || self._stats.highscore < self._stats.attributes[scoreId].value)) {
                let score = Math.max(self._stats.newHighscore || 0, self._stats.attributes[ige.game.data.settings.scoreAttributeId].value || 0);
    ...
    loadPersistentData: function () {
        ...
        let persistData = _.cloneDeep(self.persistedData);
    ...
    hideMenu: function () {
        if (ige.isClient) {
            ...
            let html = `<i class="fa fa-gamepad pr-2 py-3" aria-hidden="true"></i>Continue`;
