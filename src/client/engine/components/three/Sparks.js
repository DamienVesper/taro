...
SPARKS.Engine = {
    ...
    start: function () {
        ...
        for (let i = 0, il = this._emitters.length; i < il; i++) {
    ...
    stop: function () {
        ...
        for (let i = 0, il = _emitters.length; i < il; i++) {
...
SPARKS.Emitter.prototype = {
    ...
    step: function (emitter) {
        let time = Date.now();
        let elapsed = time - emitter._lastTime;

        if (!this._velocityVerlet) {
            // if elapsed is way higher than time step, (usually after switching tabs, or excution cached in ff)
            // we will drop cycles. perhaps set to a limit of 10 or something?
            let maxBlock = emitter._TIMESTEP * 20;
    ...
    update: function (time) {
        let len = this._counter.updateEmitter(this, time);
        ...
        let action;
        let len2 = this._particles.length;
    ...
    createParticle: function () {
        let particle = new SPARKS.Particle();
        // In future, use a Particle Factory
        let len = this._initializers.length; let i;
    ...
    removeInitializer: function (initializer) {
        let index = this._initializers.indexOf(initializer);
        if (index > -1) {
...
SPARKS.SteadyCounter.prototype.updateEmitter = function (emitter, time) {
    let targetRelease = time * this.rate + this.leftover;
    let actualRelease = Math.floor(targetRelease);
...
SPARKS.Age.prototype.update = function (emitter, particle, time) {
    ...
    {
        let t = this._easing(particle.age / particle.lifetime);
...
SPARKS.ParallelogramZone.prototype.getLocation = function () {
    let d1 = this.side1.clone().multiplyScalar(Math.random());
    let d2 = this.side2.clone().multiplyScalar(Math.random());
    d1.addSelf(d2);
...
SPARKS.CubeZone.prototype.getLocation = function () {
    ...
    let location = this.position.clone();
...
SPARKS.CubeZone.prototype.contains = function (position) {
    ...
    let diffX = position.x - startX;
    let diffY = position.y - startY;
    let diffZ = position.z - startZ;
...
SPARKS.SphereCapZone.prototype.getLocation = function () {
    let theta = Math.PI * 2 * SPARKS.Utils.random();
    let r = SPARKS.Utils.random();

    // new THREE.Vector3
    let v = SPARKS.VectorPool.get().set(r * Math.cos(theta), -1 / Math.tan(this.angle * SPARKS.Utils.DEGREE_TO_RADIAN), r * Math.sin(theta));

    // v.length = StardustMath.interpolate(0, _minRadius, 1, _maxRadius, Math.random());

    let i = this.minr - ((this.minr - this.maxr) * Math.random());
    v.multiplyScalar(i);
...
SPARKS.Utils = {
    ...
    getPerpendiculars: function (normal) {
        let p1 = this.getPerpendicular(normal);
        let p2 = normal.cross(p1);
        p2.normalize();
        return [p1, p2];
    },
    ...
    getPerpendicular: function (v)
    {
        ...
        {
            let temp = new THREE.Vector3(v.y, -v.x, 0);
