 ...
 */
let IgeEditorTranslateComponent = IgeEventingClass.extend({
            ...
            if (this._enabled) {
                if (ige._sgTreeSelected) {
                    ...
                    if (this._targetEntity.classId() == `IgeViewport`) {
                        ...
                        ige.input.on(`mouseDown`, (event) => { self._mouseDown(event); });
                        ige.input.on(`mouseMove`, (event) => { self._mouseMove(event); });
                        ige.input.on(`mouseUp`, (event) => { self._mouseUp(event); });
