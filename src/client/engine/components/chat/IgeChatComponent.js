let IgeChatComponent = IgeEventingClass.extend({
    ...
    sendChatMessage: function () {
        if (ige.isClient) {
            ...
            let gameData = ige.game && ige.game.data && ige.game.data.defaultData;
    ...
    postMessage: function (msgData) {
        ...
        if (ige.isClient) {
            ...
            if (msgData.from) {
                let player = ige.game.getPlayerByClientId(msgData.from);
                let playerName = this.escapeOutput(player._stats.name);
            ...
            $(`#chat-history`).each(function () {
                ...
                let scrollDifference = Math.abs(parseInt(this.scrollTop + this.clientHeight) - parseInt(this.scrollHeight));
                let shouldScroll = (parseInt($(this).scrollTop + $(this).clientHeight) === parseInt($(this).scrollHeight)) || scrollDifference < 2;
