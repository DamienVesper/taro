...
let IgeNetIoClient = {
    ...
    start: function (server, callback) {
        if (this._state === 3) {
            ...
            let self = this;
            let gameId = ige.client.servers[0].gameId;

            self._startCallback = callback;

            let sortedServers = [server];
            let ignoreServerIds = [server.id];
            ...
            sortedServers.reduce((p, server) => {
                ...
                p.then(() => {
                    ...
                    if (self._state < 2) {
                        ...
                        let msg = `Connecting to net.io server at "${self._url}"...`;
    ...
    connectToGS: function (url) {
        ...
        self._io.on(`disconnect`, (data) => {
            let reason = data.reason;
            let code = data.code;
            let wasClean = data.wasClean;
    ...
    _onMessageFromServer: function (data) {
        ...
        if (commandName === `_snapshot`) {
            let snapshot = _.cloneDeep(data)[1];
            let newSnapshotTimeStamp = snapshot[snapshot.length - 1][1];
