...
var IgeEngine = IgeEntity.extend({
    ...
    getIdleTimeoutMs: function () {
        let defaultValue = 5;
        let idleTimeoutHours = ige.server.tier == `0` ? ige.game.data.defaultData.privateServerIdleTimeout : 0;
        let timeoutMins = idleTimeoutHours ? idleTimeoutHours * 60 : defaultValue;
    ...
    $$: function (categoryName) {
        let returnArray = this._categoryRegister[categoryName] || new IgeArray();
    ...
    removeGraph: function (className, options) {
        if (className !== undefined) {
            let classInstance = this._graphInstances[className];
        ...
        if (fpsRate !== undefined) {
            ...
            if (this.isServer) {
                ...
                window.requestAnimFrame = function (callback, element) {
                    ...
                    setTimeout(() => { callback(new Date().getTime()); }, 1000 / fpsRate); // client will always run at 60 fps.
                };
            }
        }
    },
    ...
    updateProgress: function () {
        ...
        if (typeof (document) !== `undefined` && document.getElementById) {
            let elem = document.getElementById(`loadingProgressBar`);
            let textElem = document.getElementById(`loadingText`);

            if (elem) {
                // Calculate the width from progress
                let totalWidth = parseInt(elem.parentNode.offsetWidth);
                let currentWidth = Math.floor((totalWidth / this._texturesTotal) * (this._texturesTotal - this._texturesLoading));
        ...
        if (!ige._state) {
            ...
            if (ige.dependencyCheck()) {
                ...
                if (curTime - ige._dependencyCheckStart > this._dependencyCheckTimeout) {
                    ...
                    setTimeout(() => { ige.start(callback); }, 200);
                }
            }
        }
    },
    ...
    getCtx: function () {
        ...
        let contex = this._canvas.getContext(`webgl2`, { alpha: alpha }) || this._canvas.getContext(`webgl`, { alpha: alpha }) || this._canvas.getContext(`2d`, { alpha: alpha });
    ...
    showWebView: function (url) {
        if (ige.cocoonJs && ige.cocoonJs.detected) {
            ...
            // Load the iFrame url
            let overlay = document.getElementById(`igeOverlay`);
    ...
    _resizeEvent: function (event) {
                ...
                if (ige.isClient && ige.client.scaleMode > 0) {
                    if (ige.client.scaleMode == 1 || ige.client.scaleMode == 3) {
                        ...
                        if (ige.client.scaleMode == 1) {
                            ...
                            let aspectResolution = $(`#igeFrontBuffer`).attr(`width`) / $(`#igeFrontBuffer`).attr(`height`);
                    ...
                    let w = $(`#igeFrontBuffer`).attr(`width`);
                    let h = $(`#igeFrontBuffer`).attr(`height`);
        ...
        if (ige._showSgTree) {
            let sgTreeElem = document.getElementById(`igeSgTree`);
    ...
    updateCreateEntitySnapshot: function () {
        let allEntities = ige.$(`baseScene`)._children;
        if (allEntities) {
    ...
    sendEntityCreateSnapshot: function (clientId) {
        ...
        for (let entityId in ige.entityCreateSnapshot) {
        ...
        if (self._state) {
            ...
            if (ige.isServer) {
                if (self.now - self.lastCheckedAt > 1000) {
                    ...
                    let playerCount = ige.$$(`player`).filter((player) => { return player._stats.controlledBy == `human`; }).length;
                    ...
                    let age = self.now - ige.server.gameStartedAt;

                    let shouldLog = ige.server.logTriggers && ige.server.logTriggers.timerLogs;
                    if (shouldLog) {
    ...
    updateSceneGraph: function (ctx) {
        ...
        let arrCount; let us; let ud;
        let tickDelta = ige._tickDelta;
    ...
    getSceneGraphData: function (obj, noRef) {
        let item; let items = []; let tempItem; let tempItem2; let tempCam;
        let arr; let arrCount;
    ...
    devLog: function () {
        ...
        if (ige.env == `local`) {
            let scriptInfo = ``;
            if (ige.script) {
                let script = ige.game.data.scripts[ige.script.currentScriptId];
                scriptInfo = `Script '${(script) ? script.name : ``}' in Action '${ige.script.currentActionName}' : `;
            }
            let info = scriptInfo + (new Error()).stack.split(`\n`)[2];
