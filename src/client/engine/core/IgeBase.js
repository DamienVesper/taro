...
if (typeof (ImageData) !== `undefined`) {
    ...
    ImageData.prototype.pixelAt = function (x, y) {
        let data = this.data;
        let pixelStart = (y * this.width * 4) + (x * 4);
 ...
 * @param obj
 */
let disableContextMenu = function (obj) {
    if (obj !== null) {
...
if (typeof (window) !== `undefined`) {
    ...
    requestAnimFrame = (function () {
		...
		function (callback, element) {
		    setTimeout(() => { callback(new Date().getTime()); }, 1000 / 60);
		};
    }());
} else {
    /**
