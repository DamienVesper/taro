 ...
 */
let IgeTileMap2d = IgeEntity.extend({
    ...
    init: function (tileWidth, tileHeight) {
        ...
        if (!ige.isServer) {
            let tex = new IgeTexture(IgeTileMap2dSmartTexture);
            self.texture(tex);
        }
    ...
    scanRects: function (callback) {
        let x; let y;
        let rectArray = [];
        let mapData = this.map._mapData.clone();
    ...
    saveMap: function (layer) {
        ...
        let mapData = layer.map._mapData;
    ...
    _processTriggerHitTests: function () {
        ...
        if (this._mouseEventsActive && ige._currentViewport) {
            if (!this._mouseAlwaysInside) {
                let mouseTile = this.mouseToTile(ige._selectedViewport);
