...
const videoControlsOriginal = document.getElementById(`video-controls-my`).cloneNode(true);
document.getElementById(`video-controls-my`).remove();

let videoChatEnabled = false;
...
function addVideoStream (video, stream, peerID = null, playerName = null, from = `null`) {
    ...
    if (peerID) {
        ...
        // # clone the controls.
        const videoControls = videoControlsOriginal.cloneNode(true);
...
function handleVideoUI (el) {
    const videoID = `video-${el.getAttribute(`data-peer`)}`;
    const videoEl = document.getElementById(videoID);
