...
let Client = IgeClass.extend({
    ...
    init: function () {
        ...
        $.when(self.igeEngineStarted).done(() => {
            // SANDBOX mode
            if (typeof mode === `string` && mode == `sandbox`) {
                $.ajax({
                    ...
                    success: function (game) {
                        ...
                        // load map
                        let gameMap = ige.scaleMap(game.data.map);

                        ige.map.load(gameMap);

                        let baseTilesize = 64;
                        let tilesizeRatio = baseTilesize / ige.game.data.map.tilewidth;
                ...
                if (self.preSelectedServerId) {
                    for (let serverObj of self.servers) {
                ...
                if (!self.server) {
                    let bestServer = self.getBestServer();
    ...
    getServersArray: function () {
        let serversList = [];
        let serverOptions = $(`#server-list > option`).toArray();
    ...
    getBestServer: function (ignoreServerIds) {
        let self = this;
        let firstChoice = null; // server which has max players and is under 80% capacity
        let secondChoice = null;
        let validServers = self.servers.filter((server) => {
        ...
        let overloadCriteria = 0.8;
        let maxPlayersInUnderLoadedServer = 0;
        let minPlayerCount = Number.MAX_SAFE_INTEGER;

        for (let server of validServers) {
            let capacity = server.playerCount / server.maxPlayers;
    ...
    loadGameTextures: function () {
        return new Promise((resolve, reject) => {
            ...
            for (var key in ige.game.data.unitTypes) {
                let unit = ige.game.data.unitTypes[key];
            ...
            for (var key in ige.game.data.projectileTypes) {
                let projectile = ige.game.data.projectileTypes[key];
            ...
            for (var key in ige.game.data.itemTypes) {
                let item = ige.game.data.itemTypes[key];
            ...
            resource.load((loadedResource) => {
                for (let imageName in loadedResource.resources) {
                    let resource = loadedResource.resources[imageName];
                    resource.animation = new IgePixiAnimation();
                    if (resource && resource.url) {
                        let cellSheet = ige.client.loadedTextures[resource.name];
                        if (cellSheet) {
    ...
    initEngine: function () {
        ...
        promise.then((game) => {
            let params = ige.client.getUrlVars();
            ...
            let tileWidth = ige.scaleMapDetails.tileWidth;
            let tileHeight = ige.scaleMapDetails.tileHeight;
    ...
    connectToServer: function () {
        ige.network.start(ige.client.server, (data) => {
            ...
            if (ige.client.server) {
                let serverIP = ige.client.server.url.split(`://`)[1];
                if (serverIP) {
                    let serverName = serverIP.split(`.`)[0];
            ...
            let sendInterval = ige.game.data.settings.latency || (ige._fpsRate > 0) ? 1000 / ige._fpsRate : 70;
            ...
            ige.network.stream.on(`entityCreated`, (entity) => {
                if (entity._category == `unit`) // unit detected. add it to units array
                {
                    ...
                    if (unit._stats.ownerId) {
                        let ownerPlayer = ige.$(unit._stats.ownerId);
                        if (ownerPlayer) {
                    ...
                    if (player._stats.controlledBy === `human`) {
                        ...
                        let unitsObject = ige.game.getUnitsByClientId(player._stats.clientId);
            ...
            ige.network.stream.on(`entityDestroyed`, (unitBeingDestroyed) => {
    ...
    loadCSP: function () {
        ...
        let gravity = ige.game.data.settings.gravity;
        if (gravity) {
    ...
    joinGame: function () {
        ...
        if (!ige.mobileControls.isMobile) {
            setTimeout(() => {
                self.lowFPSInterval = setInterval(() => {
        ...
        if (!window.playerJoined) {
            ...
            window.errorLogTimer = setTimeout(() => {
                let list = ige.client.eventLog.reduce((p, e) => {
    ...
    getUrlVars: function () {
        ...
        let tempGameId = window.location.pathname.split(`/`)[2];
        let vars = {
            gameId: tempGameId
        };
    ...
    applyInactiveTabEntityStream: function () {
        let self = this;
        for (let entityId in self.inactiveTabEntityStream) {
            let entityData = _.cloneDeep(self.inactiveTabEntityStream[entityId]);
