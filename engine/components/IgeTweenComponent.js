 ...
 */
let IgeTweenComponent = IgeClass.extend({
    ...
    _setupStep: function (tween, newTime) {
        let targetObj = tween._targetObj;
        let step = tween._steps[tween._currentStep];
	 ...
	 */
    update: function (ctx) {
        let thisTween = this.tween;
        if (thisTween._tweens && thisTween._tweens.length) {
            let currentTime = ige._tickStart;
            let tweens = thisTween._tweens;
                    ...
                    if (deltaTime >= destTime) {
                        ...
                        for (targetIndex in targets) {
                            if (targets.hasOwnProperty(targetIndex)) {
                                ...
                                let roundingPrecision = Math.pow(10, 15 - (targetPropVal.toFixed(0).toString().length));
