...
THREE.TrackballControls = function (object, domElement) {
    ...
    this.rotateCamera = function () {
        ...
        if (angle) {
            let axis = (new THREE.Vector3()).cross(_rotateStart, _rotateEnd).normalize();
    ...
    this.panCamera = function () {
        let mouseChange = _panEnd.clone().subSelf(_panStart);

        if (mouseChange.lengthSq()) {
            mouseChange.multiplyScalar(_eye.length() * _this.panSpeed);

            let pan = _eye.clone().crossSelf(_this.object.up).setLength(mouseChange.x);
