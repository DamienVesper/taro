let IgeInputComponent = IgeEventingClass.extend({
    ...
    _rationalise: function (event, touch, debug) {
        ...
        if (event.igeType === `key`) {
            if (event.keyCode === 8) { // Backspace
                ...
                let elem = event.srcElement || event.target;
    ...
    _chatHandler: function (e) {
        let isChatInputHidden = $(`#chat-message-input`).css(`display`) === `none`;
        if (e.keyCode === 13) {
            ...
            let isMenuVisible = $(`#menu-wrapper`).is(`:visible`);
    ...
    actionState: function (actionName) {
        let val = this._state[this._controlMap[actionName]];
    ...
    tick: function () {
        ...
        let arr = this._eventQueue;
        let arrCount = arr.length;
        let evc = this._eventControl;
    ...
    emit: function (eventName, args) {
        if (this._eventListeners) {
            ...
            if (this._eventListeners[eventName]) {
                ...
                let evc = this._eventControl;
                let finalArgs; let i; let cancelFlag; let eventIndex; let tempEvt; let retVal;
