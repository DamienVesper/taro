let IgeNetIoServer = {
   ...
   */
    define: function (commandName, callback) {
        if (commandName !== undefined) {
            ...
            let index = this._networkCommandsIndex.length;
   ...
   */
    ...
    // console.log('SNAPSHOT CONTAINS', this.snapshot)
        ...
        let commandIndex = this._networkCommandsLookup._snapshot;
        let ciEncoded = String.fromCharCode(commandIndex);

        if (commandIndex !== undefined) {
            // send sendQueue
            if (self.sendQueue) {
                for (let clientId in self.sendQueue) {
                    // simulate lag for dev environment
                    if (global.isDev) {
                        setTimeout((data, id, ci) => {
                            self._io.send(
                                [ci, data],
   ...
   */
    ...
    // Grab the original request object
        let req = this._requests[requestId];
   ...
   */
    _onClientConnect: function (socket) {
        ...
        let remoteAddress = socket._remoteAddress;
        console.log(`client is attempting to connect`, remoteAddress);
        var reason = ``;

        let bannedIps = ige.server.bannedIps;
        // ip is banned
        let playerIsBanned = false;
        ...
        const playerCount = ige.$$(`player`).filter((player) => {
        ...
        if (shouldConnect || socket._fromPingService) {
            ...
            if (!this.emit(`connect`, socket)) {
                ...
                socket.on(`disconnect`, (data) => {
                    let isClient = self.clientIds.includes(socket.id);
    ...
    // added by Jaeyun to prevent error
        if (typeof data[0] === `string`) {
            if (data[0].charCodeAt(0) != undefined) {
                let ciDecoded = data[0].charCodeAt(0);
                let commandName = this._networkCommandsIndex[ciDecoded];
   ...
   */
    _onClientDisconnect: function (data, socket) {
        ...
        let indexToRemove = this.clientIds.findIndex((id) => {
