...
NetIo.Class = typeof (IgeClass) !== `undefined` ? IgeClass : (function () {
    ...
    /**
         ...
         */
    let log = function (text, type, obj) {
        let indent = ``;
        let i;
        let stack;
    ...
    /**
         ...
         * "componentId".
         */
    let addComponent = function (component, options) {
        let newComponent = new component(this, options);
    ...
    /**
         ...
         * copied regardless.
         */
    let implement = function (classObj, overwrite) {
        let i; let obj = classObj.prototype || classObj;
    ...
    Class.extend = function () {
        ...
        let prop = arguments[arguments.length - 1];
        let extensionArray = arguments[0];
        let extensionItem;
        let extensionOverwrite;
        let extensionIndex;
        let propertyIndex;
        let propertyObject;
    ...
    on: function (eventName, call, context, oneShot, sendEventName) {
        ...
        if (typeof call === `function`) {
            if (typeof eventName === `string`) {
                ...
                for (let i in this._eventListeners[eventName]) {
                ...
                if (eventName.length) {
                    ...
                    for (let eventIndex in eventName) {
                        let eventData = eventName[eventIndex];
                        let eventObj = eventData[0];
                        let eventNameArray = eventData[1];

                        multiEvent[0] += eventNameArray.length;

                        for (let singleEventIndex in eventNameArray) {
                            // Get the event name
                            let singleEventName = eventNameArray[singleEventIndex];
    ...
    off: function (eventName, evtListener) {
        if (this._eventListeners) {
            if (this._eventListeners[eventName]) {
                ...
                let evtListIndex = this._eventListeners[eventName].indexOf(evtListener);
                if (evtListIndex > -1) {
...
NetIo.Socket = NetIo.EventingClass.extend({
    ...
    send: function (data) {
        try {
            this._socket.send(this._encode(data), (error) => {
...
NetIo.Server = NetIo.EventingClass.extend({
    ...
    start: function (port, callback) {
        let self = this;
        this._port = port;
        let secure = true; // to turn on/off https
        ...
        // http
        this._httpServer = this._http.createServer((request, response) => {
        ...
        // Setup listener
        this._socketServerHttp.on(`connection`, (ws, request) => {
        ...
        this._httpServer.listen(this._port, (err) => {
        ...
        if (secure) {
            ...
            let privateKey = this._fs.readFileSync(`../sslcert/modd_ssl.key`, `utf8`);
            let certificate = this._fs.readFileSync(`../sslcert/modd_ssl.crt`, `utf8`);
            let options = { key: privateKey, cert: certificate };
            this._httpsServer = this._https.createServer(options, (request, response) => {
                response.writeHead(404);
                response.end();
            });
            ...
            // Setup listener
            this._socketServerHttps.on(`connection`, (ws, request) => {
                self.socketConnection(ws, request);
            });
            ...
            this._httpsServer.listen(self._portSecure, (err) => {
                if (err) {
                    console.log(err);
                }
    ...
    socketConnection: function (ws, request) {
        ...
        if (!socket._fromPingService) {
            ...
            let token = request.url && request.url.replace(`/?token=`, ``);
            try {
                let decodedToken = jwt.verify(token, `k=9PE%C&Ammu}<K`);
                socket._token = {
        ...
        socket.on(`disconnect`, (response) => {
            ...
            let index = self._sockets.indexOf(socket);
            if (index > -1) {
