...
var IgeStreamComponent = IgeEventingClass.extend({
    ...
    updateEntityAttributes: function () {
        let data = {};
        let entities = ige.$(`baseScene`)._children;

        let sendData = false;
        for (let i = 0; i < entities.length; i++) {
            var entity = entities[i];
            let queuedStreamData = entity.getQueuedStreamData();
    ...
    _onStreamCreate: function (data) {
        ...
        let entityId = data[1];
        let parentId = data[2];
        let transformData = data[3];
        let createData = data[4];
        let parent = ige.$(parentId);
        ...
        if (parent) {
            ...
            if (!ige.$(entityId)) {
                ...
                if (classConstructor) {
                    ...
                    let body = entity._stats.currentBody;
