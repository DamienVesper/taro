...
console.log(`test 2`);
let io = (typeof module === `undefined` ? {} : module.exports);
(function () {
/**
 * socket.io
 ...
 */
   ...
   */
        ...
        util.merge = function merge (target, additional, deep, lastseen) {
            let seen = lastseen || [];
            let depth = typeof deep === `undefined` ? 2 : deep;
            let prop;
   ...
   */
        ...
        util.intersect = function (arr, arr2) {
            let ret = [];
            let longest = arr.length > arr2.length ? arr : arr2;
            let shortest = arr.length > arr2.length ? arr2 : arr;
 ...
 */
   ...
   */
        ...
        let advice = parser.advice = [`reconnect`];
 ...
 */
   ...
   */
        ...
        Socket.prototype.getTransport = function (override) {
            let transports = override || this.transports; let match;
   ...
   */
        ...
        Socket.prototype.onDisconnect = function (reason) {
            ...
            let wasConnecting = this.connecting;
   ...
   */
        ...
        Socket.prototype.reconnect = function () {
            ...
            let self = this;
            let maxAttempts = this.options[`max reconnection attempts`];
            let tryMultiple = this.options[`try multiple transports`];
            let limit = this.options[`reconnection limit`];
   ...
   */
        ...
        Flashsocket.prototype.ready = function (socket, fn) {
            function init () {
                ...
                let port = options[`flash policy port`];
                let path = [
                    `http${options.secure ? `s` : ``}:/`,
                    `${options.host}:${options.port}`,
                    options.resource,
 ...
 */
   ...
   */
        ...
        XHR.prototype.request = function (method) {
            ...
            let query = io.util.query(this.socket.options.query, `t=${+new Date()}`);
   ...
   */
        ...
        XHR.check = function (socket, xdomain) {
            try {
                let request = io.util.request(xdomain);
                let usesXDomReq = (global.XDomainRequest && request instanceof XDomainRequest);
                let socketProtocol = (socket && socket.options && socket.options.secure ? `https:` : `http:`);
                let isXProtocol = (socketProtocol != global.location.protocol);
   ...
   */
        ...
        HTMLFile.check = function (socket) {
            if (typeof window !== `undefined` && ([`Active`].concat(`Object`).join(`X`)) in window) {
                try {
                    let a = new window[([`Active`].concat(`Object`).join(`X`))](`htmlfile`);
   ...
   */
        ...
        JSONPPolling.prototype.post = function (data) {
            ...
            if (!this.form) {
                ...
                let id = this.iframeId = `socketio_iframe_${this.index}`;
                var iframe;
