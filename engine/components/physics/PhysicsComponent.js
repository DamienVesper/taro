...
var PhysicsComponent = IgeEventingClass.extend({
    ...
    destroyBody: function (entity, body) {
        ...
        if (body || (entity && entity.body)) {
            ...
            let isBodyDestroyed = destroyBody.apply(this._world, [body]);
            ...
            if (this.engine == `BOX2DWEB`) {
                ...
                for (var i = 0; i < this._world.m_contactManager.m_contactFactory.m_registers.length; i++) {
                    for (let j = 0; j < this._world.m_contactManager.m_contactFactory.m_registers[i].length; j++) {
    ...
    destroyJoint: function (entityA, entityB) {
        if (entityA && entityA.body && entityB && entityB.body) {
            let joint = entityA.jointsAttached[entityB.id()];
            if (joint) {
    ...
    getBodiesInRegion: function (region) {
        ...
        if (ige.physics.engine == `crash`) {
            let collider = new self.crash.Box(new self.crash.Vector(region.x + (region.width / 2), region.y + (region.height / 2)), region.width, region.height);
    ...
    staticsFromMap: function (mapLayer, callback) {
        ...
        if (mapLayer.map) {
            let tileWidth = ige.scaleMapDetails.tileWidth || mapLayer.tileWidth();
            let tileHeight = ige.scaleMapDetails.tileHeight || mapLayer.tileHeight();
            let posX; let posY;
            ...
            while (rectCount--) {
                ...
                let wall = new IgeEntityBox2d(defaultData)
        ...
        if (self && self._active && self._world) {
            ...
            if (!self._world.isLocked()) {
                while (self._actionQueue.length > 0) {
                    let action = self._actionQueue.shift();
                    queueSize++;
                    if (queueSize > 1000) {
            ...
            if (self.engine == `crash`) { // crash's engine step happens in dist.js
                ...
                while (tempBod && typeof tempBod.getNext === `function`) {
                    ...
                    if (tempBod.m_type !== `static` && tempBod.isAwake()) {
                        ...
                        if (entity) {
                            let mxfp = dists[ige.physics.engine].getmxfp(tempBod);
                            let x = mxfp.x * ige.physics._scaleRatio;
                            let y = mxfp.y * ige.physics._scaleRatio;
                            ...
                            let skipBoundaryCheck = entity._stats && entity._stats.confinedWithinMapBoundaries === false;
                            let padding = tileWidth / 2;
                            ...
                            if (ige.isServer) {
                                ...
                                if (ige.game.cspEnabled && entity.clientStreamedPosition) {
                                    let targetX = entity.clientStreamedPosition[0];
                                    let targetY = entity.clientStreamedPosition[1];
                                    var xDiff = targetX - x;
                                    var yDiff = targetY - y;
