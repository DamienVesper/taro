...
(function (factory) {
    ...
    let SEARCH_OBJECT = {}; // will hold the AABB coordinates during search()
    let ALL_MOVED = []; // will hold all the colliders that have moved during check(), so we can update their lastCheckedPos

    let Crash = function (options) {
        if (!options) {
            options = {};
        }
    ...
    Crash.prototype.__onCollision = function (a, b, res) {
        for (let i = 0, len = this.__listeners.length; i < len; i++) {
    ...
    Crash.prototype.search = function (collider) {
        ...
        let res = this.rbush.search(SEARCH_OBJECT);
    ...
    ****************/
    ...
    Crash.updateAABBPolygon = Crash.prototype.updateAABBPolygon = function (collider) {
        ...
        let pos = collider.sat.pos;
        let points = collider.sat.calcPoints;
    ...
    Crash.updateAABBCircle = Crash.prototype.updateAABBCircle = function (collider) {
        let aabb = collider.aabb;
        let r = collider.sat.r;
        let center = collider.sat.pos;
    ...
    Crash.test = Crash.prototype.test = function (a, b, res) {
        ...
        let str = this.getTestString(a.type, b.type);
    ...
    Crash.prototype.check = function (res) {
        ...
        while (this.__moved.length && i < this.MAX_CHECKS) {
            let collider = this.__moved.pop();
            let index = ALL_MOVED.indexOf(collider);
            if (index === -1) {
    ...
    Crash.createColliders = Crash.prototype.createColliders = function (crash) {
        let Collider = crash.Collider = function Collider (type, sat, insert, data) {
            this.type = type;
            this.sat = sat;
            this.data = data;
        ...
        let Polygon = crash.Polygon = function Polygon (pos, points, insert, data) {
            let sat = new SAT.Polygon(pos, points);
        ...
        let Circle = crash.Circle = function Circle (pos, r, insert, data) {
            let sat = new SAT.Circle(pos, r);
        ...
        let Point = crash.Point = function Point (pos, insert, data) {
            let sat = (new SAT.Box(pos, 1, 1)).toPolygon();
        ...
        let Box = crash.Box = function Box (pos, w, h, insert, data) {
            let sat = (new SAT.Box(pos, w, h)).toPolygon();
