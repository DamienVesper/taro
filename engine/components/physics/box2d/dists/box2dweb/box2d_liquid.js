...
let b2Profiler = (function ()
{
    ...
    let profiles = [];
    let root = new profileStruct(`root`);
    ...
    function create (name, parent)
    {
        ...
        let s = new profileStruct(name, parent || `root`);
        profiles.push(s);
        return s;
    }
...
let b2_maxFloat		= Number.MAX_VALUE;
let b2_epsilon		= 2.2204460492503131e-016;
let b2_pi			= Math.PI;
...
/// This is in meters.
let b2_aabbExtension			= 0.1;

/// This is used to fatten AABBs in the dynamic tree. This is used to predict
/// the future position based on the current displacement.
/// This is a dimensionless multiplier.
let b2_aabbMultiplier			= 2.0;

/// A small length used as a collision and constraint tolerance. Usually it is
/// chosen to be numerically significant, but visually insignificant.
let b2_linearSlop				= 0.005;

/// A small angle used as a collision and constraint tolerance. Usually it is
/// chosen to be numerically significant, but visually insignificant.
let b2_angularSlop				= (2.0 / 180.0 * b2_pi);

/// The radius of the polygon/edge shape skin. This should not be modified. Making
/// this smaller means polygons will have an insufficient buffer for continuous collision.
/// Making it larger may create artifacts for vertex collision.
let b2_polygonRadius			= (2.0 * b2_linearSlop);

/// Maximum number of sub-steps per contact in continuous physics simulation.
let b2_maxSubSteps				= 8;

// Dynamics

/// Maximum number of contacts to be handled to solve a TOI impact.
let b2_maxTOIContacts			= 32;

/// A velocity threshold for elastic collisions. Any collision with a relative linear
/// velocity below this threshold will be treated as inelastic.
let b2_velocityThreshold		= 1.0;

/// The maximum linear position correction used when solving constraints. This helps to
/// prevent overshoot.
let b2_maxLinearCorrection		= 0.2;

/// The maximum angular position correction used when solving constraints. This helps to
/// prevent overshoot.
let b2_maxAngularCorrection		= (8.0 / 180.0 * b2_pi);

/// The maximum linear velocity of a body. This limit is very large and is used
/// to prevent numerical problems. You shouldn't need to adjust this.
let b2_maxTranslation			= 2.0;
let b2_maxTranslationSquared	= (b2_maxTranslation * b2_maxTranslation);

/// The maximum angular velocity of a body. This limit is very large and is used
/// to prevent numerical problems. You shouldn't need to adjust this.
let b2_maxRotation				= (0.5 * b2_pi);
let b2_maxRotationSquared		= (b2_maxRotation * b2_maxRotation);

/// This scale factor controls how fast overlap is resolved. Ideally this would be 1 so
/// that overlap is removed in one time step. However using values close to 1 often lead
/// to overshoot.
let b2_baumgarte				= 0.2;
let b2_toiBaugarte				= 0.75;
...
let b2_linearSleepTolerance		= 0.01;

/// A body cannot sleep if its angular velocity is above this tolerance.
let b2_angularSleepTolerance	= (2.0 / 180.0 * b2_pi);
...
/// Current version.
let b2_version = new b2Version(2, 3, 1);
/*
 * Copyright (c) 2006-2009 Erin Catto http://www.physics.org
 *
 * This software is provided 'as-is', without any express or implied
...
let sqrtf = Math.sqrt;
let atan2f = Math.atan2;
let sinf = Math.sin;
let cosf = Math.cos;
let floorf = Math.floor;
let ceilf = Math.ceil;
...
b2Vec2.prototype =
{
    ...
    /// Negate this vector.
    Negate: function () { let v = new b2Vec2(); v.Set(-this.x, -this.y); return v; },
...
b2Vec3.prototype =
{
    ...
    /// Negate this vector.
    Negate: function () { let v = new b2Vec3(); v.Set(-this.x, -this.y, -this.z); return v; },
...
b2Mat22.prototype =
{
    ...
    GetInverse: function ()
    {
        let a = this.ex.x; let b = this.ey.x; let c = this.ex.y; let d = this.ey.y;
        let B = new b2Mat22();
        let det = a * d - b * c;
        if (det != 0.0)
        {
    ...
    Solve: function (b)
    {
        let a11 = this.ex.x; let a12 = this.ey.x; let a21 = this.ex.y; let a22 = this.ey.y;
...
b2Mat33.prototype =
{
    ...
    Solve33: function (b)
    {
        let det = b2Dot_v3_v3(this.ex, b2Cross_v3_v3(this.ey, this.ez));
        if (det != 0.0)
        {
            det = 1.0 / det;
        }
    ...
    GetSymInverse33: function (/* b2Mat33* */ M)
    {
        ...
        let a11 = this.ex.x; let a12 = this.ey.x; let a13 = this.ez.x;
        let a22 = this.ey.y; let a23 = this.ez.y;
        let a33 = this.ez.z;
...
b2Transform.prototype =
{
    Clone: function ()
    {
        let xf = new b2Transform(this.p, this.q);
        return xf;
    },
...
b2Sweep.prototype =
{
    ...
    {
        let twoPi = 2.0 * b2_pi;
        let d = twoPi * floorf(this.a0 / twoPi);
        this.a0 -= d;
        this.a -= d;
    },
...
{
    let c1 = new b2Vec2(b2Dot_v2_v2(A.ex, B.ex), b2Dot_v2_v2(A.ey, B.ex));
    let c2 = new b2Vec2(b2Dot_v2_v2(A.ex, B.ey), b2Dot_v2_v2(A.ey, B.ey));
...
b2CircleShape.prototype =
{
    ...
    {
        let center = b2Vec2.Add(transform.p, b2Mul_r_v2(transform.q, this.m_p));
        let d = b2Vec2.Subtract(p, center);
    ...
    RayCast: function (output, input,
        transform, childIndex)
    {
        let position = b2Vec2.Add(transform.p, b2Mul_r_v2(transform.q, this.m_p));
        let s = b2Vec2.Subtract(input.p1, position);
        let b = b2Dot_v2_v2(s, s) - this.m_radius * this.m_radius;

        // Solve quadratic equation.
        let r = b2Vec2.Subtract(input.p2, input.p1);
        let c = b2Dot_v2_v2(s, r);
        let rr = b2Dot_v2_v2(r, r);
        let sigma = c * c - rr * b;
    ...
    {
        let px = transform.p.x + (transform.q.c * this.m_p.x - transform.q.s * this.m_p.y);// b2Vec2.Add(transform.p, b2Mul_r_v2(transform.q, this.m_p));
        let py = transform.p.y + (transform.q.s * this.m_p.x + transform.q.c * this.m_p.y);
...
b2EdgeShape.prototype =
{
        ...
        xf, childIndex)
    {
        ...
        let p1 = b2MulT_r_v2(xf.q, b2Vec2.Subtract(input.p1, xf.p));
        let p2 = b2MulT_r_v2(xf.q, b2Vec2.Subtract(input.p2, xf.p));
        let d = b2Vec2.Subtract(p2, p1);

        let v1 = this.m_vertex1;
        let v2 = this.m_vertex2;
        let e = b2Vec2.Subtract(v2, v1);
        let normal = new b2Vec2(e.y, -e.x);
        normal.Normalize();

        // q = p1 + t * d
        // dot(normal, q - v1) = 0
        // dot(normal, p1 - v1) + t * dot(normal, d) = 0
        let numerator = b2Dot_v2_v2(normal, b2Vec2.Subtract(v1, p1));
        let denominator = b2Dot_v2_v2(normal, d);
        ...
        let q = b2Vec2.Add(p1, b2Vec2.Multiply(t, d));

        // q = v1 + s * r
        // s = dot(q - v1, r) / dot(r, r)
        let r = b2Vec2.Subtract(v2, v1);
        let rr = b2Dot_v2_v2(r, r);
        if (rr == 0.0)
        {
            return false;
        ...
        let s = b2Dot_v2_v2(b2Vec2.Subtract(q, v1), r) / rr;
        if (s < 0.0 || s > 1.0)
        {
            return false;
        }
    ...
    ComputeAABB: function (aabb, xf, childIndex)
    {
        let v1x = (xf.q.c * this.m_vertex1.x - xf.q.s * this.m_vertex1.y) + xf.p.x;// b2Mul_t_v2(xf, this.m_vertex1);
        let v1y = (xf.q.s * this.m_vertex1.x + xf.q.c * this.m_vertex1.y) + xf.p.y;
        let v2x = (xf.q.c * this.m_vertex2.x - xf.q.s * this.m_vertex2.y) + xf.p.x;// b2Mul_t_v2(xf, this.m_vertex2);
        let v2y = (xf.q.s * this.m_vertex2.x + xf.q.c * this.m_vertex2.y) + xf.p.y;

        let lowerx = b2Min(v1x, v2x);// = b2Min_v2(v1, v2);
        let lowery = b2Min(v1y, v2y);
        let upperx = b2Max(v1x, v2x); //= b2Max_v2(v1, v2);
        let uppery = b2Max(v1y, v2y);
...
b2ChainShape.prototype =
{
    ...
    ComputeAABB: function (aabb, xf, childIndex)
    {
        ...
        let v1x = (xf.q.c * this.m_vertices[i1].x - xf.q.s * this.m_vertices[i1].y) + xf.p.x;// b2Mul_t_v2(xf, this.m_vertices[i1]);
        let v1y = (xf.q.s * this.m_vertices[i1].x + xf.q.c * this.m_vertices[i1].y) + xf.p.y;
        let v2x = (xf.q.c * this.m_vertices[i2].x - xf.q.s * this.m_vertices[i2].y) + xf.p.x;// b2Mul_t_v2(xf, this.m_vertices[i2]);
        let v2y = (xf.q.s * this.m_vertices[i2].x + xf.q.c * this.m_vertices[i2].y) + xf.p.y;
...
b2PolygonShape.prototype =
{
    ...
    Set: function (vertices, count)
    {
        ...
        let ps = new Array(b2_maxPolygonVertices);
        let tempCount = 0;
        ...
        let hull = new Array(b2_maxPolygonVertices);
        let m = 0;
        let ih = i0;
        ...
        for (;;)
        {
            ...
            for (j = 1; j < n; ++j)
            {
                ...
                let r = b2Vec2.Subtract(ps[ie], ps[hull[m]]);
                var v = b2Vec2.Subtract(ps[j], ps[hull[m]]);
                let c = b2Cross_v2_v2(r, v);
                if (c < 0.0)
                {
                    ie = j;
                }
        ...
        for (i = 0; i < m; ++i)
        {
            let i1 = i;
            let i2 = i + 1 < m ? i + 1 : 0;
            let edge = b2Vec2.Subtract(this.m_vertices[i2], this.m_vertices[i1]);
    ...
    TestPoint: function (xf, p)
    {
        let pLocal = b2MulT_r_v2(xf.q, b2Vec2.Subtract(p, xf.p));

        for (let i = 0; i < this.m_count; ++i)
        {
            let dot = b2Dot_v2_v2(this.m_normals[i], b2Vec2.Subtract(pLocal, this.m_vertices[i]));
            if (dot > 0.0)
            {
                return false;
            }
        ...
        xf, childIndex)
    {
        ...
        let lower = 0.0; let upper = input.maxFraction;
        ...
        for (let i = 0; i < this.m_count; ++i)
        {
            ...
            let numerator = b2Dot_v2_v2(this.m_normals[i], b2Vec2.Subtract(this.m_vertices[i], p1));
            let denominator = b2Dot_v2_v2(this.m_normals[i], d);
    ...
    ComputeAABB: function (aabb, xf, childIndex)
    {
        let lowerx = (xf.q.c * this.m_vertices[0].x - xf.q.s * this.m_vertices[0].y) + xf.p.x;// b2Mul_t_v2(xf, this.m_vertices[0]);
        let lowery = (xf.q.s * this.m_vertices[0].x + xf.q.c * this.m_vertices[0].y) + xf.p.y;
        let upperx = lowerx;
        let uppery = lowery;

        for (let i = 1; i < this.m_count; ++i)
        {
            let vx = (xf.q.c * this.m_vertices[i].x - xf.q.s * this.m_vertices[i].y) + xf.p.x;// b2Mul_t_v2(xf, this.m_vertices[i]);
            let vy = (xf.q.s * this.m_vertices[i].x + xf.q.c * this.m_vertices[i].y) + xf.p.y;
    ...
    ComputeMass: function (massData, density)
    {
        ...
        {
            // Triangle vertices.
            let e1 = b2Vec2.Subtract(this.m_vertices[i], s);
            let e2 = i + 1 < this.m_count ? b2Vec2.Subtract(this.m_vertices[i + 1], s) : b2Vec2.Subtract(this.m_vertices[0], s);

            let D = b2Cross_v2_v2(e1, e2);
            ...
            let ex1 = e1.x; let ey1 = e1.y;
            let ex2 = e2.x; let ey2 = e2.y;
    ...
    Validate: function ()
    {
        ...
        {
            let i1 = i;
            let i2 = i < this.m_count - 1 ? i1 + 1 : 0;
            let p = this.m_vertices[i1];
            let e = b2Vec2.Subtract(this.m_vertices[i2], p);
            ...
            for (let j = 0; j < this.m_count; ++j)
            {
                ...
                let v = b2Vec2.Subtract(this.m_vertices[j], p);
                let c = b2Cross_v2_v2(e, v);
                if (c < 0.0)
                {
                    return false;
                }
...
b2PolygonShape.ComputeCentroid = function (vs, count)
{
    ...
    for (let i = 0; i < count; ++i)
    {
        ...
        let e1 = b2Vec2.Subtract(p2, p1);
        let e2 = b2Vec2.Subtract(p3, p1);
...
b2DistanceProxy.prototype =
{
    ...
    {
        let bestIndex = 0;
        let bestValue = this.m_vertices[0].x * dx + this.m_vertices[0].y * dy;// b2Dot_v2_v2(this.m_vertices[0], d);
        for (let i = 1; i < this.m_count; ++i)
        {
            let value = this.m_vertices[i].x * dx + this.m_vertices[i].y * dy;// b2Dot_v2_v2(this.m_vertices[i], d);
            if (value > bestValue)
            {
                bestIndex = i;
                bestValue = value;
...
b2Simplex.prototype =
{
    ...
    GetSearchDirection: function (p)
    {
        switch (this.m_count)
        {
            ...
            case 2:
                {
                    let e12x = this.m_v[1].w.x - this.m_v[0].w.x;// b2Vec2.Subtract(this.m_v[1].w, this.m_v[0].w);
                    let e12y = this.m_v[1].w.y - this.m_v[0].w.y;
                    let sgn = e12x * -this.m_v[0].w.y - e12y * -this.m_v[0].w.x;// b2Cross_v2_v2(e12, this.m_v[0].w.Negate());
                    if (sgn > 0.0)
                    {
    ...
    Solve2: function ()
    {
        ...
        // w1 region
        let d12_2 = -(w1.x * e12x + w1.y * e12y);// -b2Dot_v2_v2(w1, e12);
        if (d12_2 <= 0.0)
        {
        ...
        // w2 region
        let d12_1 = w2.x * e12x + w2.y * e12y;// b2Dot_v2_v2(w2, e12);
        if (d12_1 <= 0.0)
        {
    ...
    Solve3: function ()
    {
        ...
        let e12x = w2.x - w1.x;
        let e12y = w2.y - w1.y;
        let w1e12 = w1.x * e12x + w1.y * e12y;// b2Dot_v2_v2(w1, e12);
        let w2e12 = w2.x * e12x + w2.y * e12y;// b2Dot_v2_v2(w2, e12);
        let d12_1 = w2e12;
        let d12_2 = -w1e12;
        ...
        let e13x = w3.x - w1.x;
        let e13y = w3.y - w1.y;
        let w1e13 = w1.x * e13x + w1.y * e13y;// b2Dot_v2_v2(w1, e13);
        let w3e13 = w3.x * e13x + w3.y * e13y;// b2Dot_v2_v2(w3, e13);
        let d13_1 = w3e13;
        let d13_2 = -w1e13;
        ...
        let e23x = w3.x - w2.x;
        let e23y = w3.y - w2.y;
        let w2e23 = w2.x * e23x + w2.y * e23y;// b2Dot_v2_v2(w2, e23);
        let w3e23 = w3.x * e23x + w3.y * e23y;// b2Dot_v2_v2(w3, e23);
        let d23_1 = w3e23;
        let d23_2 = -w2e23;

        // Triangle123
        let n123 = e12x * e13y - e12y * e13x;// b2Cross_v2_v2(e12, e13);

        let d123_1 = n123 * (w2.x * w3.y - w2.y * w3.x);// b2Cross_v2_v2(w2, w3);
        let d123_2 = n123 * (w3.x * w1.y - w3.y * w1.x);// b2Cross_v2_v2(w3, w1);
        let d123_3 = n123 * (w1.x * w2.y - w1.y * w2.x);// b2Cross_v2_v2(w1, w2);
...
let _b2Distance_simplex = new b2Simplex();
let _b2Distance_normal = new b2Vec2();
let _b2Distance_p = new b2Vec2();
    ...
    input)
{
    ...
    let vertices = _b2Distance_simplex.m_v;
    let k_maxIters = 20;

    // These store the vertices of the last simplex so that we
    // can check for duplicates and prevent cycling.
    let saveA = [0, 0, 0]; let saveB = [0, 0, 0];
    let saveCount = 0;
    ...
    while (iter < k_maxIters)
    {
        ...
        let vertex = vertices[_b2Distance_simplex.m_count];
        ...
        let pva = proxyA.GetVertex(vertex.indexA);
        ...
        let pvb = proxyB.GetVertex(vertex.indexB);
    ...
    if (input.useRadii)
    {
        ...
        else
        {
            ...
            let px = (0.5 * (output.pointA.x + output.pointB.x));
            let py = (0.5 * (output.pointA.y + output.pointB.y));
            ...
            case b2Manifold.e_circles:
                {
                    ...
                    let pointAx = (xfA.q.c * manifold.localPoint.x - xfA.q.s * manifold.localPoint.y) + xfA.p.x;// b2Mul_t_v2(xfA, manifold.localPoint);
                    let pointAy = (xfA.q.s * manifold.localPoint.x + xfA.q.c * manifold.localPoint.y) + xfA.p.y;
                    let pointBx = (xfB.q.c * manifold.points[0].localPoint.x - xfB.q.s * manifold.points[0].localPoint.y) + xfB.p.x;// b2Mul_t_v2(xfB, manifold.points[0].localPoint);
                    let pointBy = (xfB.q.s * manifold.points[0].localPoint.x + xfB.q.c * manifold.points[0].localPoint.y) + xfB.p.y;

                    let cx = pointAx - pointBx;
                    let cy = pointAy - pointBy;
...
b2AABB.prototype =
{
    ...
    RayCast: function (output, input)
    {
        ...
        let p = input.p1;
        let d = b2Vec2.Subtract(input.p2, input.p1);
        let absD = b2Abs_v2(d);
        ...
        for (let i = 0; i < 2; ++i)
        {
            ...
            {
                let inv_d = 1.0 / d.get_i(i);
                let t1 = (this.lowerBound.get_i(i) - p.get_i(i)) * inv_d;
                let t2 = (this.upperBound.get_i(i) - p.get_i(i)) * inv_d;
    ...
    circleB, xfB)
{
    ...
    let pA = b2Mul_t_v2(xfA, circleA.m_p);
    let pB = b2Mul_t_v2(xfB, circleB.m_p);

    let dx = pB.x - pA.x;// b2Vec2.Subtract(pB, pA);
    let dy = pB.y - pA.y;
    let distSqr = dx * dx + dy * dy;// b2Dot_v2_v2(d, d);
    let rA = circleA.m_radius; let rB = circleB.m_radius;
    let radius = rA + rB;
    ...
    circleB, xfB)
{
    ...
    let c = b2Mul_t_v2(xfB, circleB.m_p);
    let cLocal = b2MulT_t_v2(xfA, c);
    ...
    {
        let s = normals[i].x * (cLocal.x - vertices[i].x) + normals[i].y * (cLocal.y - vertices[i].y);// b2Dot_v2_v2(normals[i], b2Vec2.Subtract(cLocal, vertices[i]));
    ...
    let u1 = (cLocal.x - v1.x) * (v2.x - v1.x) + (cLocal.y - v1.y) * (v2.y - v1.y);// b2Dot_v2_v2(b2Vec2.Subtract(cLocal, v1), b2Vec2.Subtract(v2, v1));
    let u2 = (cLocal.x - v2.x) * (v1.x - v2.x) + (cLocal.y - v2.y) * (v1.y - v2.y);// b2Dot_v2_v2(b2Vec2.Subtract(cLocal, v2), b2Vec2.Subtract(v1, v2));
    if (u1 <= 0.0)
    {
    ...
    {
        let faceCenterx = 0.5 * (v1.x + v2.x);// b2Vec2.Multiply(0.5, b2Vec2.Add(v1, v2));
    ...
    poly2, xf2)
{
    ...
    for (let i = 0; i < count1; ++i)
    {
        ...
        let nx = xf.q.c * n1s[i].x - xf.q.s * n1s[i].y;// b2Mul_r_v2(xf.q, n1s[i]);
        let ny = xf.q.s * n1s[i].x + xf.q.c * n1s[i].y;
        let v1x = (xf.q.c * v1s[i].x - xf.q.s * v1s[i].y) + xf.p.x;// b2Mul_t_v2(xf, v1s[i]);
        let v1y = (xf.q.s * v1s[i].x + xf.q.c * v1s[i].y) + xf.p.y;

        // Find deepest point for normal i.
        let si = b2_maxFloat;
        for (let j = 0; j < count2; ++j)
        {
            let sij = nx * (v2s[j].x - v1x) + ny * (v2s[j].y - v1y);// b2Dot_v2_v2(n, b2Vec2.Subtract(v2s[j], v1));
            if (sij < si)
            {
                si = sij;
            }
    ...
    poly2, xf2)
{
    ...
    let t1x = xf1.q.c * normals1[edge1].x - xf1.q.s * normals1[edge1].y;
    let t1y = xf1.q.s * normals1[edge1].x + xf1.q.c * normals1[edge1].y;
    let normal1x = xf2.q.c * t1x + xf2.q.s * t1y;
    let normal1y = -xf2.q.s * t1x + xf2.q.c * t1y;
    ...
    {
        let dot = normal1x * normals2[i].x + normal1y * normals2[i].y;// b2Dot_v2_v2(normal1, normals2[i]);
        if (dot < minDot)
        {
            minDot = dot;
            index = i;
    ...
    polyB, xfB)
{
    ...
    let edgeA = [0];
    let separationA = b2FindMaxSeparation(edgeA, polyA, xfA, polyB, xfB);
    if (separationA > totalRadius)
        return;

    let edgeB = [0];
    let separationB = b2FindMaxSeparation(edgeB, polyB, xfB, polyA, xfA);
    ...
    let localNormalx = 1.0 * b2CollidePolygons._localTangent.y;// b2Cross_v2_f(b2CollidePolygons._localTangent, 1.0);
    let localNormaly = -1.0 * b2CollidePolygons._localTangent.x;
    let planePointx = 0.5 * (v11.x + v12.x);// b2Vec2.Multiply(0.5, b2Vec2.Add(v11, v12));
    let planePointy = 0.5 * (v11.y + v12.y);

    let tangentx = xf1.q.c * b2CollidePolygons._localTangent.x - xf1.q.s * b2CollidePolygons._localTangent.y;// b2Mul_r_v2(xf1.q, b2CollidePolygons._localTangent);
    let tangenty = xf1.q.s * b2CollidePolygons._localTangent.x + xf1.q.c * b2CollidePolygons._localTangent.y;
    let normalx = 1.0 * tangenty;// b2Cross_v2_f(tangent, 1.0);
    let normaly = -1.0 * tangentx;

    v11 = b2Mul_t_v2(xf1, v11);
    v12 = b2Mul_t_v2(xf1, v12);

    // Face offset.
    let frontOffset = normalx * v11.x + normaly * v11.y;// b2Dot_v2_v2(normal, v11);

    // Side offsets, extended by polytope skin thickness.
    let sideOffset1 = /* -b2Dot_v2_v2(tangent, v11) */-(tangentx * v11.x + tangenty * v11.y) + totalRadius;
    let sideOffset2 = /* b2Dot_v2_v2(tangent, v12) */(tangentx * v12.x + tangenty * v12.y) + totalRadius;

    // Clip incident edge against extruded edge1 side edges.
    let clipPoints1 = new Array(2);
    let clipPoints2 = new Array(2);
    let np;
    ...
    {
        let separation = /* b2Dot_v2_v2(normal, clipPoints2[i].v) */(normalx * clipPoints2[i].v.x + normaly * clipPoints2[i].v.y) - frontOffset;

        if (separation <= totalRadius)
        {
            let cp = manifold.points[pointCount] = new b2ManifoldPoint();
            cp.localPoint.Assign(b2MulT_t_v2(xf2, clipPoints2[i].v));
            cp.id.Assign(clipPoints2[i].id);
            if (flip)
            {
                // Swap features
                let cf = new b2ContactID();
                cf.Assign(cp.id);
    ...
    circleB, xfB)
{
    ...
    let Q = b2MulT_t_v2(xfA, b2Mul_t_v2(xfB, circleB.m_p));

    let A = edgeA.m_vertex1; let B = edgeA.m_vertex2;
    let ex = B.x - A.x;// b2Vec2.Subtract(B, A);
    let ey = B.y - A.y;

    // Barycentric coordinates
    let u = ex * (B.x - Q.x) + ey * (B.y - Q.y);// b2Dot_v2_v2(e, b2Vec2.Subtract(B, Q));
    let v = ex * (Q.x - A.x) + ey * (Q.y - A.y);// b2Dot_v2_v2(e, b2Vec2.Subtract(Q, A));
    ...
    if (v <= 0.0)
    {
        ...
        if (edgeA.m_hasVertex0)
        {
            ...
            let B1 = A;
            let e1x = B1.x - A1.x;// b2Vec2.Subtract(B1, A1);
            let e1y = B1.y - A1.y;
            let u1 = e1x * (B1.x - Q.x) + e1y * (B1.y - Q.y);// b2Dot_v2_v2(e1, b2Vec2.Subtract(B1, Q));
    ...
    if (u <= 0.0)
    {
        ...
        if (edgeA.m_hasVertex3)
        {
            ...
            let A2 = B;
            let e2x = B2.x - A2.x;// b2Vec2.Subtract(B2, A2);
            let e2y = B2.y - A2.y;
            let v2 = e2x * (Q.x - A2.x) + e2y * (Q.y - A2.y);// b2Dot_v2_v2(e2, b2Vec2.Subtract(Q, A2));
    ...
    // Region AB
    let den = ex * ex + ey * ey;// b2Dot_v2_v2(e, e);

    b2Assert(den > 0.0);

    let Px = (1.0 / den) * ((u * A.x) + (v * B.x));
    let Py = (1.0 / den) * ((u * A.y) + (v * B.y));// b2Vec2.Multiply((1.0 / den), b2Vec2.Add(b2Vec2.Multiply(u, A), b2Vec2.Multiply(v, B)));
...
b2EPCollider.prototype =
{
        ...
        polygonB, xfB)
    {
        ...
        let offset1 = this.m_normal1.x * (this.m_centroidB.x - this.m_v1.x) + this.m_normal1.y * (this.m_centroidB.y - this.m_v1.y);
        let offset0 = 0.0; let offset2 = 0.0;
        let convex1 = false; let convex2 = false;
        ...
        if (primaryAxis.type == b2EPAxis.e_edgeA)
        {
            ...
            let bestIndex = 0;
            let bestValue = this.m_normal.x * this.m_polygonB.normals[0].x + this.m_normal.y * this.m_polygonB.normals[0].y;// b2Dot_v2_v2(this.m_normal, this.m_polygonB.normals[0]);
            for (var i = 1; i < this.m_polygonB.count; ++i)
            {
                let value = this.m_normal.x * this.m_polygonB.normals[i].x + this.m_normal.y * this.m_polygonB.normals[i].y;// b2Dot_v2_v2(this.m_normal, this.m_polygonB.normals[i]);
                if (value < bestValue)
                {
                    bestValue = value;
                    bestIndex = i;
        ...
        for (var i = 0; i < b2_maxManifoldPoints; ++i)
        {
            ...
            let separation = rf.normal.x * (clipPoints2[i].v.x - rf.v1.x) + rf.normal.y * (clipPoints2[i].v.y - rf.v1.y);
    ...
    ComputeEdgeSeparation: function ()
    {
        ...
        for (let i = 0; i < this.m_polygonB.count; ++i)
        {
            ...
            let s = this.m_normal.x * (this.m_polygonB.vertices[i].x - this.m_v1.x) + this.m_normal.y * (this.m_polygonB.vertices[i].y - this.m_v1.y);
    ...
    ComputePolygonSeparation: function ()
    {
        ...
        let perpx = -this.m_normal.y;// = new b2Vec2(-this.m_normal.y, this.m_normal.x);
        let perpy = this.m_normal.x;

        for (let i = 0; i < this.m_polygonB.count; ++i)
        {
            let nx = -this.m_polygonB.normals[i].x;// this.m_polygonB.normals[i].Negate();
            let ny = -this.m_polygonB.normals[i].y;

            // var s1 = b2Dot_v2_v2(n, b2Vec2.Subtract(this.m_polygonB.vertices[i], this.m_v1));
            let s1 = nx * (this.m_polygonB.vertices[i].x - this.m_v1.x) + ny * (this.m_polygonB.vertices[i].y - this.m_v1.y);
            // var s2 = b2Dot_v2_v2(n, b2Vec2.Subtract(this.m_polygonB.vertices[i], this.m_v2));
            let s2 = nx * (this.m_polygonB.vertices[i].x - this.m_v2.x) + ny * (this.m_polygonB.vertices[i].y - this.m_v2.y);
            let s = b2Min(s1, s2);
    ...
    normalx, normaly, offset, vertexIndexA)
{
    ...
    let distance0 = /* b2Dot_v2_v2(normal, vIn[0].v) */(normalx * vIn[0].v.x + normaly * vIn[0].v.y) - offset;
    let distance1 = /* b2Dot_v2_v2(normal, vIn[1].v) */(normalx * vIn[1].v.x + normaly * vIn[1].v.y) - offset;
...
b2DynamicTree.prototype =
{
    ...
    MoveProxy: function (proxyId, aabb, displacement)
    {
        ...
        let d = b2Vec2.Multiply(b2_aabbMultiplier, displacement);
    ...
    RayCast: function (callback, input)
    {
        ...
        let v = b2Cross_f_v2(1.0, r);
        let abs_v = b2Abs_v2(v);
        ...
        while (stack.length > 0)
        {
            ...
            let separation = b2Abs(b2Dot_v2_v2(v, b2Vec2.Subtract(p1, c))) - b2Dot_v2_v2(abs_v, h);
            if (separation > 0.0)
            {
                continue;
            }
    ...
    {
        let maxBalance = 0;
        for (let i = 0; i < this.m_nodeCapacity; ++i)
        {
            ...
            let balance = b2Abs(this.m_nodes[child2].height - this.m_nodes[child1].height);
    ...
    GetAreaRatio: function ()
    {
        ...
        let rootArea = root.aabb.GetPerimeter();
    ...
    {
        let nodes = new Array(this.m_nodeCount);
        let count = 0;
    ...
    InsertLeaf: function (leaf)
    {
        ...
        while (this.m_nodes[index].IsLeaf() == false)
        {
            ...
            let area = this.m_nodes[index].aabb.GetPerimeter();

            let combinedAABB = new b2AABB();
            combinedAABB.Combine(this.m_nodes[index].aabb, leafAABB);
            let combinedArea = combinedAABB.GetPerimeter();
    ...
    RemoveLeaf: function (leaf)
    {
        ...
        let parent = this.m_nodes[leaf].parent;
    ...
    ComputeHeight: function (nodeId)
    {
        ...
        let height1 = this.ComputeHeight(node.child1);
        let height2 = this.ComputeHeight(node.child2);
    ...
    ValidateMetrics: function (index)
    {
        ...
        let height1 = this.m_nodes[child1].height;
        let height2 = this.m_nodes[child2].height;
        let height;
...
let _local_xfA = new b2Transform();
let _local_xfB = new b2Transform();
    ...
    FindMinSeparation: function (indices, t)
    {
        ...
        switch (this.m_type)
        {
            case b2SeparationFunction.e_points:
            {
                ...
                let axisAy = -_local_xfA.q.s * this.m_axis.x + _local_xfA.q.c * this.m_axis.y;
...
let profile_toi = b2Profiler.create(`toi`, `solveTOI`);
function b2TimeOfImpact (output, input)
{
    ...
    let target = b2Max(b2_linearSlop, totalRadius - 3.0 * b2_linearSlop);
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            let indices = [];
            let s2 = fcn.FindMinSeparation(indices, t2);
            ...
            let s1 = fcn.Evaluate(indices[0], indices[1], t1);
            ...
            for (;;)
            {
                ...
                let s = fcn.Evaluate(indices[0], indices[1], t);
...
b2Body.prototype =
{
    ...
    ResetMassData: function ()
    {
        ...
        let localCenter = new b2Vec2(0, 0);
        for (let f = this.m_fixtureList; f; f = f.m_next)
        {
            if (f.m_density == 0.0)
            {
    ...
    {
        let status = (this.m_flags & b2Body.e_fixedRotationFlag) == b2Body.e_fixedRotationFlag;
        if (status == flag)
        {
            return;
        }
...
b2Fixture.prototype =
{
    ...
    Refilter: function ()
    {
        ...
        let world = this.m_body.GetWorld();
    ...
    Create: function (body, def)
    {
        ...
        // Reserve proxy space
        let childCount = this.m_shape.GetChildCount();
    ...
    Synchronize: function (broadPhase, transform1, transform2)
    {
        ...
        for (let i = 0; i < this.m_proxyCount; ++i)
        {
            ...
            let aabb1 = new b2AABB(); let aabb2 = new b2AABB();
            ...
            let displacement = b2Vec2.Subtract(transform2.p, transform1.p);
...
b2ContactFilter.prototype =
{
    ...
    {
        let filterA = fixtureA.GetFilterData();
        let filterB = fixtureB.GetFilterData();
...
let profile_world_step = b2Profiler.create(`step`);
let profile_world_collide = b2Profiler.create(`collide`, `step`);
let profile_world_solve = b2Profiler.create(`solve`, `step`);
let profile_world_solveTOI = b2Profiler.create(`solveTOI`, `step`);
let profile_world_broadphase = b2Profiler.create(`broadphase`, `step`);
...
b2WorldRayCastWrapper.prototype =
{
    ...
    {
        let userData = this.broadPhase.GetUserData(proxyId);
        let proxy = userData;
        ...
        if (hit)
        {
            ...
            let point = b2Vec2.Add(b2Vec2.Multiply((1.0 - fraction), input.p1), b2Vec2.Multiply(fraction, input.p2));
...
b2World.prototype =
{
    ...
    DrawDebugData: function ()
    {
        ...
        let flags = this.g_debugDraw.GetFlags();
        ...
        {
            for (let j = this.m_jointList; j; j = j.GetNext())
            {
                this.DrawJoint(j);
            }
        }
        ...
        if (flags & b2Draw.e_pairBit)
        {
            ...
            for (let c = this.m_contactManager.m_contactList; c; c = c.GetNext())
            {
                let fixtureA = c.GetFixtureA();
                let fixtureB = c.GetFixtureB();

                let cA = fixtureA.GetAABB(c.GetChildIndexA()).GetCenter();
                let cB = fixtureB.GetAABB(c.GetChildIndexB()).GetCenter();
        ...
        if (flags & b2Draw.e_aabbBit)
        {
            ...
            let bp = this.m_contactManager.m_broadPhase;
    ...
    Solve: function (step)
    {
        ...
        for (let seed = this.m_bodyList; seed; seed = seed.m_next)
        {
            ...
            // Post solve cleanup.
            for (let i = 0; i < this.p_island.m_bodyCount; ++i)
            {
        ...
        for (;;)
        {
            ...
            for (var c = this.m_contactManager.m_contactList; c; c = c.m_next)
            {
                ...
                else
                {
                    ...
                    let activeA = bA.IsAwake() && typeA != b2Body.b2_staticBody;
                    let activeB = bB.IsAwake() && typeB != b2Body.b2_staticBody;
                    ...
                    let collideA = bA.IsBullet() || typeA != b2Body.b2_dynamicBody;
                    let collideB = bB.IsBullet() || typeB != b2Body.b2_dynamicBody;
    ...
    DrawJoint: function (joint)
    {
        ...
        let color = new b2Color(0.5, 0.8, 0.8);
            ...
            case b2Shape.e_circle:
                {
                    ...
                    let center = b2Mul_t_v2(xf, circle.m_p);
                    let radius = circle.m_radius;
                    let axis = b2Mul_r_v2(xf.q, new b2Vec2(1.0, 0.0));
...
b2Contact.prototype =
{
    ...
    Update: function (listener)
    {
        ...
        let touching = false;
        let wasTouching = (this.m_flags & b2Contact.e_touchingFlag) == b2Contact.e_touchingFlag;
        ...
        else
        {
            ...
            for (let i = 0; i < this.m_manifold.pointCount; ++i)
            {
                ...
                for (let j = 0; j < b2Contact.m_local_tempManifold.pointCount; ++j)
                {
                    let mp1 = b2Contact.m_local_tempManifold.points[j];
...
let _local_temp_edgeShape = new b2EdgeShape();
...
b2ChainAndCircleContact.prototype =
{
    Evaluate: function (manifold, xfA, xfB)
    {
        let chain = this.m_fixtureA.GetShape();
...
b2Contact.Create = function (fixtureA, indexA, fixtureB, indexB)
{
    ...
    let fcn = b2Contact.s_registers[type1][type2].fcn;

    if (fcn)
    {
        let contact = b2Contact.RetrieveGarbage(fcn);
...
b2Contact.Destroy = function (contact)
{
    ...
    let typeA = fixtureA.GetType();
    let typeB = fixtureB.GetType();
...
let b2_defaultListener = new b2ContactListener();
...
b2ContactManager.prototype =
{
    ...
    AddPair: function (proxyUserDataA, proxyUserDataB)
    {
        ...
        while (edge)
        {
            if (edge.other == bodyA)
            {
                ...
                let iA = edge.contact.GetChildIndexA();
                let iB = edge.contact.GetChildIndexB();
    ...
    Collide: function ()
    {
        ...
        while (c)
        {
            ...
            let activeA = bodyA.IsAwake() && bodyA.m_type != b2Body.b2_staticBody;
            let activeB = bodyB.IsAwake() && bodyB.m_type != b2Body.b2_staticBody;
...
b2PositionSolverManifold.prototype =
{
    Initialize: function (pc, xfA, xfB, index)
    {
        ...
        switch (pc.type)
        {
            case b2Manifold.e_circles:
                {
                    let pointAx = (xfA.q.c * pc.localPoint.x - xfA.q.s * pc.localPoint.y) + xfA.p.x;
                    let pointAy = (xfA.q.s * pc.localPoint.x + xfA.q.c * pc.localPoint.y) + xfA.p.y;// b2Mul_t_v2(xfA, pc.localPoint);
                    let pointBx = (xfB.q.c * pc.localPoints[0].x - xfB.q.s * pc.localPoints[0].y) + xfB.p.x;
                    let pointBy = (xfB.q.s * pc.localPoints[0].x + xfB.q.c * pc.localPoints[0].y) + xfB.p.y;// b2Mul_t_v2(xfB, pc.localPoints[0]);
                    ...
                    this.normal.y = pointBy - pointAy;
                    let tempnx = this.normal.x;
                    let tempny = this.normal.y;
                    this.normal.Normalize();
...
b2ContactSolver.prototype =
{
    Init: function (def)
    {
        ...
        for (let i = 0; i < this.m_count; ++i)
        {
            ...
            let vc = this.m_velocityConstraints[i] || new b2ContactVelocityConstraint();
            ...
            let pc = this.m_positionConstraints[i] || new b2ContactPositionConstraint();
    ...
    InitializeVelocityConstraints: function ()
    {
        for (let i = 0; i < this.m_count; ++i)
        {
            ...
            let cA = this.m_positions[indexA].c;
            let aA = this.m_positions[indexA].a;
            let vA = this.m_velocities[indexA].v;
            let wA = this.m_velocities[indexA].w;

            let cB = this.m_positions[indexB].c;
            let aB = this.m_positions[indexB].a;
            let vB = this.m_velocities[indexB].v;
            ...
            for (let j = 0; j < pointCount; ++j)
            {
                ...
                let rnA = vcp.rA.x * vc.normal.y - vcp.rA.y * vc.normal.x;// b2Cross_v2_v2(vcp.rA, vc.normal);
                let rnB = vcp.rB.x * vc.normal.y - vcp.rB.y * vc.normal.x;// b2Cross_v2_v2(vcp.rB, vc.normal);

                let kNormal = mA + mB + iA * rnA * rnA + iB * rnB * rnB;

                vcp.normalMass = kNormal > 0.0 ? 1.0 / kNormal : 0.0;

                let tangentx = 1.0 * vc.normal.y;// b2Cross_v2_f(vc.normal, 1.0);
                let tangenty = -1.0 * vc.normal.x;

                let rtA = vcp.rA.x * tangenty - vcp.rA.y * tangentx;// b2Cross_v2_v2(vcp.rA, tangent);
                let rtB = vcp.rB.x * tangenty - vcp.rB.y * tangentx;// b2Cross_v2_v2(vcp.rB, tangent);
                ...
                let vRel = vc.normal.x * (((vB.x + (-wB * vcp.rB.y)) - vA.x) - (-wA * vcp.rA.y)) + vc.normal.y * (((vB.y + (wB * vcp.rB.x)) - vA.y) - (wA * vcp.rA.x));
            ...
            if (vc.pointCount == 2)
            {
                ...
                let rn1A = vcp1.rA.x * vc.normal.y - vcp1.rA.y * vc.normal.x;// b2Cross_v2_v2(vcp1.rA, vc.normal);
                let rn1B = vcp1.rB.x * vc.normal.y - vcp1.rB.y * vc.normal.x;// b2Cross_v2_v2(vcp1.rB, vc.normal);
                let rn2A = vcp2.rA.x * vc.normal.y - vcp2.rA.y * vc.normal.x;// b2Cross_v2_v2(vcp2.rA, vc.normal);
                let rn2B = vcp2.rB.x * vc.normal.y - vcp2.rB.y * vc.normal.x;// b2Cross_v2_v2(vcp2.rB, vc.normal);
                ...
                let k_maxConditionNumber = 1000.0;
    ...
    WarmStart: function ()
    {
        ...
        for (let i = 0; i < this.m_count; ++i)
        {
            ...
            let tangentx = 1.0 * normal.y;
            let tangenty = -1.0 * normal.x;// b2Cross_v2_f(normal, 1.0);

            for (let j = 0; j < pointCount; ++j)
            {
                let vcp = vc.points[j];
                let Px = (vcp.normalImpulse * normal.x) + (vcp.tangentImpulse * tangentx);
                let Py = (vcp.normalImpulse * normal.y) + (vcp.tangentImpulse * tangenty);
    ...
    SolveVelocityConstraints: function ()
    {
        for (let i = 0; i < this.m_count; ++i)
        {
            ...
            let normal = vc.normal;
            let tangentx = 1.0 * normal.y;// b2Cross_v2_f(normal, 1.0);
            let tangenty = -1.0 * normal.x;
            ...
            for (let j = 0; j < pointCount; ++j)
            {
                ...
                let vt = (dvx * tangentx + dvy * tangenty) /* b2Dot_v2_v2(dv, tangent) */ - vc.tangentSpeed;
            ...
            if (vc.pointCount == 1)
            {
                ...
                let vn = dvx * normal.x + dvy * normal.y;// b2Dot_v2_v2(dv, normal);
            ...
            else
            {
                ...
                let vn1 = dv1x * normal.x + dv1y * normal.y;// b2Dot_v2_v2(dv1, normal);
                let vn2 = dv2x * normal.x + dv2y * normal.y;// b2Dot_v2_v2(dv2, normal);
                ...
                for (;;)
                {
                    ...
                    // x = - inv(A) * b'
                    //
                    let xx = -(vc.normalMass.ex.x * bx + vc.normalMass.ey.x * by);
                    let xy = -(vc.normalMass.ex.y * bx + vc.normalMass.ey.y * by);
    ...
    SolvePositionConstraints: function ()
    {
        ...
        for (let i = 0; i < this.m_count; ++i)
        {
            ...
            for (let j = 0; j < pointCount; ++j)
            {
                ...
                let normal = b2ContactSolver.temp_solver_manifold.normal;

                let point = b2ContactSolver.temp_solver_manifold.point;
                let separation = b2ContactSolver.temp_solver_manifold.separation;

                let rAx = point.x - cA.x;
                let rAy = point.y - cA.y; // b2Vec2.Subtract(point, cA);
                let rBx = point.x - cB.x; // b2Vec2.Subtract(point, cB);
                let rBy = point.y - cB.y;
                ...
                let rnA = rAx * normal.y - rAy * normal.x;// b2Cross_v2_v2(rA, normal);
                let rnB = rBx * normal.y - rBy * normal.x;// b2Cross_v2_v2(rB, normal);
                ...
                let Py = impulse * normal.y;// b2Vec2.Multiply(impulse, normal);
    ...
    SolveTOIPositionConstraints: function (toiIndexA, toiIndexB)
    {
        ...
        for (let i = 0; i < this.m_count; ++i)
        {
            ...
            for (let j = 0; j < pointCount; ++j)
            {
                ...
                let rA = b2Vec2.Subtract(point, cA);
                let rB = b2Vec2.Subtract(point, cB);
                ...
                // Compute the effective mass.
                let rnA = b2Cross_v2_v2(rA, normal);
                let rnB = b2Cross_v2_v2(rB, normal);
                let K = mA + mB + iA * rnA * rnA + iB * rnB * rnB;

                // Compute normal impulse
                let impulse = K > 0.0 ? -C / K : 0.0;

                let P = b2Vec2.Multiply(impulse, normal);
...
let profile_solve_init = b2Profiler.create(`solve initialization`, `solve`);
let profile_solve_init_warmStarting = b2Profiler.create(`warm starting`, `solve initialization`);
let profile_solve_velocity = b2Profiler.create(`solve velocities`, `solve`);
let profile_solve_position = b2Profiler.create(`solve positions`, `solve`);
...
b2Island.prototype =
{
    ...
    Solve: function (step, gravity, allowSleep)
    {
        ...
        for (var i = 0; i < this.m_bodyCount; ++i)
        {
            let c = this.m_positions[i].c;
            var a = this.m_positions[i].a;
            let v = this.m_velocities[i].v;
            var w = this.m_velocities[i].w;

            // Check for large velocities
            let translationx = h * v.x;
            let translationy = h * v.y;
            let translationl = translationx * translationx + translationy * translationy;
        ...
        {
            let contactsOkay = b2Island._solver.SolvePositionConstraints();

            let jointsOkay = true;
            for (var j = 0; j < this.m_jointCount; ++j)
            {
                let jointOkay = this.m_joints[j].SolvePositionConstraints(b2Island._solverData);
    ...
    SolveTOI: function (subStep, toiIndexA, toiIndexB)
    {
        ...
        {
            let contactsOkay = b2Island._solver.SolveTOIPositionConstraints(toiIndexA, toiIndexB);
            if (contactsOkay)
            {
                break;
            }
        ...
        for (var i = 0; i < this.m_bodyCount; ++i)
        {
            ...
            // Check for large velocities
            let translation = b2Vec2.Multiply(h, v);
...
b2RevoluteJoint.prototype =
{
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        let vA = data.velocities[this.m_indexA].v.Clone();
        let wA = data.velocities[this.m_indexA].w;

        let aB = data.positions[this.m_indexB].a;
        let vB = data.velocities[this.m_indexB].v.Clone();
        let wB = data.velocities[this.m_indexB].w;

        let qA = new b2Rot(aA); let qB = new b2Rot(aB);
        ...
        let mA = this.m_invMassA; let mB = this.m_invMassB;
        let iA = this.m_invIA; let iB = this.m_invIB;
    ...
    SolveVelocityConstraints: function (data)
    {
        ...
        {
            let Cdot1 = b2Vec2.Subtract(b2Vec2.Subtract(b2Vec2.Add(vB, b2Cross_f_v2(wB, this.m_rB)), vA), b2Cross_f_v2(wA, this.m_rA));
            let Cdot2 = wB - wA;
    ...
    SolvePositionConstraints: function (data)
    {
        let cA = data.positions[this.m_indexA].c.Clone();
        let aA = data.positions[this.m_indexA].a;
        let cB = data.positions[this.m_indexB].c.Clone();
        let aB = data.positions[this.m_indexB].a;
        ...
        // Solve point-to-point constraint.
        {
            qA.Set(aA);
            qB.Set(aB);
            let rA = b2Mul_r_v2(qA, b2Vec2.Subtract(this.m_localAnchorA, this.m_localCenterA));
            let rB = b2Mul_r_v2(qB, b2Vec2.Subtract(this.m_localAnchorB, this.m_localCenterB));
...
b2MouseJoint.prototype =
{
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        let mass = this.m_bodyB.GetMass();

        // Frequency
        let omega = 2.0 * b2_pi * this.m_frequencyHz;
    ...
    SolveVelocityConstraints: function (data)
    {
        ...
        // Cdot = v + cross(w, r)
        let Cdot = b2Vec2.Add(vB, b2Cross_f_v2(wB, this.m_rB));
        let impulse = b2Mul_m22_v2(this.m_mass, (b2Vec2.Add(b2Vec2.Add(Cdot, this.m_C), b2Vec2.Multiply(this.m_gamma, this.m_impulse))).Negate());

        let oldImpulse = this.m_impulse.Clone();
        this.m_impulse.Add(impulse);
...
b2DistanceJointDef.prototype =
{
        ...
        anchor1, anchor2)
    {
        ...
        let d = b2Vec2.Subtract(anchor2, anchor1);
...
b2DistanceJoint.prototype =
{
    ...
    GetReactionForce: function (inv_dt)
    {
        let F = b2Vec2.Multiply((inv_dt * this.m_impulse), this.m_u);
        return F;
    },
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        let crAu = b2Cross_v2_v2(this.m_rA, this.m_u);
        let crBu = b2Cross_v2_v2(this.m_rB, this.m_u);
        ...
        if (data.step.warmStarting)
        {
            ...
            let P = b2Vec2.Multiply(this.m_impulse, this.m_u);
    ...
    SolveVelocityConstraints: function (data)
    {
        ...
        let vpA = b2Vec2.Add(vA, b2Cross_f_v2(wA, this.m_rA));
        let vpB = b2Vec2.Add(vB, b2Cross_f_v2(wB, this.m_rB));
        let Cdot = b2Dot_v2_v2(this.m_u, b2Vec2.Subtract(vpB, vpA));

        let impulse = -this.m_mass * (Cdot + this.m_bias + this.m_gamma * this.m_impulse);
        this.m_impulse += impulse;

        let P = b2Vec2.Multiply(impulse, this.m_u);
        vA.Subtract(b2Vec2.Multiply(this.m_invMassA, P));
        wA -= this.m_invIA * b2Cross_v2_v2(this.m_rA, P);
        vB.Add(b2Vec2.Multiply(this.m_invMassB, P));
        wB += this.m_invIB * b2Cross_v2_v2(this.m_rB, P);
    ...
    SolvePositionConstraints: function (data)
    {
        ...
        let rA = b2Mul_r_v2(qA, b2Vec2.Subtract(this.m_localAnchorA, this.m_localCenterA));
        let rB = b2Mul_r_v2(qB, b2Vec2.Subtract(this.m_localAnchorB, this.m_localCenterB));
        let u = b2Vec2.Subtract(b2Vec2.Subtract(b2Vec2.Add(cB, rB), cA), rA);
        ...
        let P = b2Vec2.Multiply(impulse, u);
...
b2PrismaticJoint.prototype =
{
    ...
    GetJointTranslation: function ()
    {
        ...
        let d = b2Vec2.Subtract(pB, pA);
        let axis = this.m_bodyA.GetWorldVector(this.m_localXAxisA);

        let translation = b2Dot_v2_v2(d, axis);
        return translation;
    },
    ...
    GetJointSpeed: function ()
    {
        ...
        let rA = b2Mul_r_v2(bA.m_xf.q, b2Vec2.Subtract(this.m_localAnchorA, bA.m_sweep.localCenter));
        let rB = b2Mul_r_v2(bB.m_xf.q, b2Vec2.Subtract(this.m_localAnchorB, bB.m_sweep.localCenter));
        let p1 = b2Vec2.Add(bA.m_sweep.c, rA);
        let p2 = b2Vec2.Add(bB.m_sweep.c, rB);
        let d = b2Vec2.Subtract(p2, p1);
        let axis = b2Mul_r_v2(bA.m_xf.q, this.m_localXAxisA);
        ...
        let speed = b2Dot_v2_v2(d, b2Cross_f_v2(wA, axis)) + b2Dot_v2_v2(axis, b2Vec2.Subtract(b2Vec2.Subtract(b2Vec2.Add(vB, b2Cross_f_v2(wB, rB)), vA), b2Cross_f_v2(wA, rA)));
        return speed;
    },
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        let rA = b2Mul_r_v2(qA, b2Vec2.Subtract(this.m_localAnchorA, this.m_localCenterA));
        let rB = b2Mul_r_v2(qB, b2Vec2.Subtract(this.m_localAnchorB, this.m_localCenterB));
        let d = b2Vec2.Add(b2Vec2.Subtract(cB, cA), b2Vec2.Subtract(rB, rA));
        ...
        {
            let jointTranslation = b2Dot_v2_v2(this.m_axis, d);
        ...
        if (data.step.warmStarting)
        {
            ...
            let P = b2Vec2.Add(b2Vec2.Multiply(this.m_impulse.x, this.m_perp), b2Vec2.Multiply((this.m_motorImpulse + this.m_impulse.z), this.m_axis));
    ...
    SolveVelocityConstraints: function (data)
    {
        ...
        if (this.m_enableLimit && this.m_limitState != b2Joint.e_inactiveLimit)
        {
            ...
            let b = b2Vec2.Subtract(Cdot1.Negate(), b2Vec2.Multiply((this.m_impulse.z - f1.z), new b2Vec2(this.m_K.ez.x, this.m_K.ez.y)));
            let f2r = b2Vec2.Add(this.m_K.Solve22(b), new b2Vec2(f1.x, f1.y));
    ...
    SolvePositionConstraints: function (data)
    {
        ...
        let rA = b2Mul_r_v2(qA, b2Vec2.Subtract(this.m_localAnchorA, this.m_localCenterA));
        let rB = b2Mul_r_v2(qB, b2Vec2.Subtract(this.m_localAnchorB, this.m_localCenterB));
        let d = b2Vec2.Subtract(b2Vec2.Subtract(b2Vec2.Add(cB, rB), cA), rA);

        let axis = b2Mul_r_v2(qA, this.m_localXAxisA);
        let a1 = b2Cross_v2_v2(b2Vec2.Add(d, rA), axis);
        let a2 = b2Cross_v2_v2(rB, axis);
        let perp = b2Mul_r_v2(qA, this.m_localYAxisA);

        let s1 = b2Cross_v2_v2(b2Vec2.Add(d, rA), perp);
        let s2 = b2Cross_v2_v2(rB, perp);
        ...
        let linearError = b2Abs(C1.x);
        let angularError = b2Abs(C1.y);

        let active = false;
        let C2 = 0.0;
        if (this.m_enableLimit)
        {
            let translation = b2Dot_v2_v2(axis, d);
            if (b2Abs(this.m_upperTranslation - this.m_lowerTranslation) < 2.0 * b2_linearSlop)
            {
                // Prevent large angular corrections
        ...
        else
        {
            ...
            let impulse1 = K.Solve(C1.Negate());
            impulse.x = impulse1.x;
            impulse.y = impulse1.y;
            impulse.z = 0.0;
        }

        let P = b2Vec2.Add(b2Vec2.Multiply(impulse.x, perp), b2Vec2.Multiply(impulse.z, axis));
...
b2WeldJoint.prototype =
{
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        if (this.m_frequencyHz > 0.0)
        {
            ...
            // Damping coefficient
            let d = 2.0 * m * this.m_dampingRatio * omega;
    ...
    SolveVelocityConstraints: function (data)
    {
        ...
        if (this.m_frequencyHz > 0.0)
        {
            ...
            let impulse1 = b2Mul22_m33_v2(this.m_mass, Cdot1).Negate();
        ...
        else
        {
            ...
            let impulse = b2Mul_m33_v3(this.m_mass, Cdot).Negate();
            this.m_impulse.Add(impulse);
    ...
    SolvePositionConstraints: function (data)
    {
        ...
        else
        {
            ...
            let impulse = K.Solve33(C).Negate();
            var P = new b2Vec2(impulse.x, impulse.y);
...
b2WheelJoint.prototype =
{
    ...
    GetJointTranslation: function ()
    {
        ...
        let pA = bA.GetWorldPoint(this.m_localAnchorA);
        let pB = bB.GetWorldPoint(this.m_localAnchorB);
        let d = b2Vec2.Subtract(pB, pA);
        let axis = bA.GetWorldVector(this.m_localXAxisA);
    ...
    GetJointSpeed: function ()
    {
        let wA = this.m_bodyA.m_angularVelocity;
        let wB = this.m_bodyB.m_angularVelocity;
        return wB - wA;
    },
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        if (this.m_frequencyHz > 0.0)
        {
            ...
            let invMass = mA + mB + iA * this.m_sAx * this.m_sAx + iB * this.m_sBx * this.m_sBx;

            if (invMass > 0.0)
            {
                this.m_springMass = 1.0 / invMass;

                let C = b2Dot_v2_v2(d, this.m_ax);
                ...
                // Spring stiffness
                let k = this.m_springMass * omega * omega;
        ...
        if (data.step.warmStarting)
        {
            ...
            let P = b2Vec2.Add(b2Vec2.Multiply(this.m_impulse, this.m_ay), b2Vec2.Multiply(this.m_springImpulse, this.m_ax));
            let LA = this.m_impulse * this.m_sAy + this.m_springImpulse * this.m_sAx + this.m_motorImpulse;
            let LB = this.m_impulse * this.m_sBy + this.m_springImpulse * this.m_sBx + this.m_motorImpulse;
    ...
    SolvePositionConstraints: function (data)
    {
        ...
        let ay = b2Mul_r_v2(qA, this.m_localYAxisA);

        let sAy = b2Cross_v2_v2(b2Vec2.Add(d, rA), ay);
        let sBy = b2Cross_v2_v2(rB, ay);

        let C = b2Dot_v2_v2(d, ay);

        let k = this.m_invMassA + this.m_invMassB + this.m_invIA * this.m_sAy * this.m_sAy + this.m_invIB * this.m_sBy * this.m_sBy;
        ...
        let P = b2Vec2.Multiply(impulse, ay);
        let LA = impulse * sAy;
        let LB = impulse * sBy;
...
function b2GearJoint (def)
{
    ...
    else
    {
        ...
        let pA = b2MulT_r_v2(xfC.q, b2Vec2.Add(b2Mul_r_v2(xfA.q, this.m_localAnchorA), b2Vec2.Subtract(xfA.p, xfC.p)));
    ...
    else
    {
        ...
        let pB = b2MulT_r_v2(xfD.q, b2Vec2.Add(b2Mul_r_v2(xfB.q, this.m_localAnchorB), b2Vec2.Subtract(xfB.p, xfD.p)));
...
b2GearJoint.prototype =
{
    ...
    GetReactionForce: function (inv_dt)
    {
        let P = b2Vec2.Multiply(this.m_impulse, this.m_JvAC);
        return b2Vec2.Multiply(inv_dt, P);
    },
    GetReactionTorque: function (inv_dt)
    {
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        let vC = data.velocities[this.m_indexC].v.Clone();
        let wC = data.velocities[this.m_indexC].w;

        let aD = data.positions[this.m_indexD].a;
        let vD = data.velocities[this.m_indexD].v.Clone();
        let wD = data.velocities[this.m_indexD].w;

        let qA = new b2Rot(aA); let qB = new b2Rot(aB); let qC = new b2Rot(aC); let qD = new b2Rot(aD);
        ...
        else
        {
            ...
            let rC = b2Mul_r_v2(qC, b2Vec2.Subtract(this.m_localAnchorC, this.m_lcC));
            let rA = b2Mul_r_v2(qA, b2Vec2.Subtract(this.m_localAnchorA, this.m_lcA));
        ...
        else
        {
            ...
            let rD = b2Mul_r_v2(qD, b2Vec2.Subtract(this.m_localAnchorD, this.m_lcD));
            let rB = b2Mul_r_v2(qB, b2Vec2.Subtract(this.m_localAnchorB, this.m_lcB));
    ...
    SolveVelocityConstraints: function (data)
    {
        ...
        let Cdot = b2Dot_v2_v2(this.m_JvAC, b2Vec2.Subtract(vA, vC)) + b2Dot_v2_v2(this.m_JvBD, b2Vec2.Subtract(vB, vD));
    ...
    SolvePositionConstraints: function (data)
    {
        ...
        let cB = data.positions[this.m_indexB].c.Clone();
        let aB = data.positions[this.m_indexB].a;
        let cC = data.positions[this.m_indexC].c.Clone();
        let aC = data.positions[this.m_indexC].a;
        let cD = data.positions[this.m_indexD].c.Clone();
        let aD = data.positions[this.m_indexD].a;
        ...
        let JvAC = new b2Vec2(); let JvBD = new b2Vec2();
        let JwA, JwB, JwC, JwD;
        let mass = 0.0;
        ...
        else
        {
            ...
            let pC = b2Vec2.Subtract(this.m_localAnchorC, this.m_lcC);
            let pA = b2MulT_r_v2(qC, b2Vec2.Add(rA, b2Vec2.Subtract(cA, cC)));
            coordinateA = b2Dot_v2_v2(b2Vec2.Subtract(pA, pC), this.m_localAxisC);
        }
        ...
        else
        {
            ...
            let pD = b2Vec2.Subtract(this.m_localAnchorD, this.m_lcD);
            let pB = b2MulT_r_v2(qD, b2Vec2.Add(rB, b2Vec2.Subtract(cB, cD)));
            coordinateB = b2Dot_v2_v2(b2Vec2.Subtract(pB, pD), this.m_localAxisD);
        }
...
b2MotorJointDef.prototype =
{
    ...
    Initialize: function (bA, bB)
    {
        this.bodyA = bA;
        this.bodyB = bB;
        let xB = this.bodyB.GetPosition();
        this.linearOffset.Assign(this.bodyA.GetLocalPoint(xB));

        let angleA = this.bodyA.GetAngle();
        let angleB = this.bodyB.GetAngle();
...
b2PulleyJointDef.prototype =
{
        ...
        r)
    {
        ...
        let dA = b2Vec2.Subtract(anchorA, groundA);
        this.lengthA = dA.Length();
        let dB = b2Vec2.Subtract(anchorB, groundB);
...
b2PulleyJoint.prototype =
{
    ...
    GetReactionForce: function (inv_dt)
    {
        let P = b2Vec2.Multiply(this.m_impulse, this.m_uB);
        return b2Vec2.Multiply(inv_dt, P);
    },
    GetReactionTorque: function (inv_dt)
    {
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        // Compute effective mass.
        let ruA = b2Cross_v2_v2(this.m_rA, this.m_uA);
        let ruB = b2Cross_v2_v2(this.m_rB, this.m_uB);
        ...
        if (data.step.warmStarting)
        {
            ...
            // Warm starting.
            let PA = b2Vec2.Multiply(-(this.m_impulse), this.m_uA);
            let PB = b2Vec2.Multiply((-this.m_ratio * this.m_impulse), this.m_uB);
    ...
    SolveVelocityConstraints: function (data)
    {
        ...
        let Cdot = -b2Dot_v2_v2(this.m_uA, vpA) - this.m_ratio * b2Dot_v2_v2(this.m_uB, vpB);
        let impulse = -this.m_mass * Cdot;
        this.m_impulse += impulse;

        let PA = b2Vec2.Multiply(-impulse, this.m_uA);
        let PB = b2Vec2.Multiply(-this.m_ratio, b2Vec2.Multiply(impulse, this.m_uB));
        vA.Add(b2Vec2.Multiply(this.m_invMassA, PA));
        wA += this.m_invIA * b2Cross_v2_v2(this.m_rA, PA);
        vB.Add(b2Vec2.Multiply(this.m_invMassB, PB));
        wB += this.m_invIB * b2Cross_v2_v2(this.m_rB, PB);
    ...
    SolvePositionConstraints: function (data)
    {
        ...
        // Get the pulley axes.
        let uA = b2Vec2.Add(cA, b2Vec2.Subtract(rA, this.m_groundAnchorA));
        let uB = b2Vec2.Add(cB, b2Vec2.Subtract(rB, this.m_groundAnchorB));
        ...
        // Compute effective mass.
        let ruA = b2Cross_v2_v2(rA, uA);
        let ruB = b2Cross_v2_v2(rB, uB);
        ...
        let linearError = b2Abs(C);

        let impulse = -mass * C;

        let PA = b2Vec2.Multiply(-impulse, uA);
        let PB = b2Vec2.Multiply(-this.m_ratio, b2Vec2.Multiply(impulse, uB));
...
b2RopeJoint.prototype =
{
    ...
    InitVelocityConstraints: function (data)
    {
        ...
        // Compute effective mass.
        let crA = b2Cross_v2_v2(this.m_rA, this.m_u);
        let crB = b2Cross_v2_v2(this.m_rB, this.m_u);
        let invMass = this.m_invMassA + this.m_invIA * crA * crA + this.m_invMassB + this.m_invIB * crB * crB;
...
b2Rope.prototype =
{
    ...
    Initialize: function (def)
    {
        ...
        for (var i = 0; i < count3; ++i)
        {
            ...
            let d1 = b2Vec2.Subtract(p2, p1);
            let d2 = b2Vec2.Subtract(p3, p2);

            let a = b2Cross_v2_v2(d1, d2);
            let b = b2Dot_v2_v2(d1, d2);
    ...
    SolveC3: function ()
    {
        ...
        for (let i = 0; i < count3; ++i)
        {
            ...
            let Jd1 = b2Vec2.Multiply((-1.0 / L1sqr), d1.Skew());
            let Jd2 = b2Vec2.Multiply((1.0 / L2sqr), d2.Skew());

            let J1 = b2Vec2.Negate(Jd1);
            let J2 = b2Vec2.Subtract(Jd1, Jd2);
            let J3 = Jd2;

            let mass = m1 * b2Dot_v2_v2(J1, J1) + m2 * b2Dot_v2_v2(J2, J2) + m3 * b2Dot_v2_v2(J3, J3);
            if (mass == 0.0)
            {
                continue;
            }
...
let b2JsonSerializer =
{
    ...
    deserialize: function (serialized, world, clear)
    {
        ...
        if (clear)
        {
            ...
            for (let j = world.GetJointList(); j;)
            {
                var next = j.GetNext();
        ...
        {
            let shapeData = deserialized.shapes[i];
            var shape;
        ...
        {
            let fixtureData = deserialized.fixtures[i];
        ...
        {
            let bodyData = deserialized.bodies[i];
            ...
            for (let x = 0; x < bodyData.fixtures.length; ++x)
        ...
        {
            let jointData = deserialized.joints[i];
            var jointDef;
...
let b2RUBELoader = (function ()
{
    ...
    {
        let vals = new Array(obj.x.length);
    ...
    function parseFixture (obj, body)
    {
        ...
        if (obj.customProperties)
            for (let i = 0; i < obj.customProperties.length; ++i)
    ...
    function parseJoint (obj, world, bodies)
    {
        ...
        let jd = new jointsList[obj.type]();
    ...
    {
        let params = new b2RubeParameters();
        ...
        if (jl)
        {
            ...
            {
                let joint = parseJoint(jl[i], world, bodies);
                joints.push(joint);
