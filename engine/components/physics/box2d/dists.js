let dists = {
    ...
    PLANCK: {
        ...
        createJoint: function (self, entityA, entityB, anchorA, anchorB) {
            ...
            let aBody = entityA._stats.currentBody;
            let bBody = entityB._stats.currentBody;
            ...
            if (
                ...
                if (aBody.jointType == `revoluteJoint` && anchorA && anchorB) {
                    let localAnchorA = planck.Vec2(anchorA.x / aBody.width, anchorA.y / aBody.height);
                    let localAnchorB = planck.Vec2(anchorB.x / bBody.width, anchorB.y / bBody.height);
                ...
                let joint = self._world.createJoint(joint_def); // joint between two pieces
        ...
        contactListener: function (self, beginContactCallback, endContactCallback, preSolve, postSolve) {
            if (beginContactCallback) {
                self._world.on(`begin-contact`, (contact, oldManifold) => {
                    beginContactCallback(contact);
                });
            }

            if (endContactCallback) {
                self._world.on(`end-contact`, (contact, oldManifold) => {
                    endContactCallback(contact);
                });
            }

            if (preSolve) {
                self._world.on(`pre-solve`, (contact, oldManifold) => {
                    preSolve(contact);
                });
            }

            if (postSolve) {
                self._world.on(`post-solve`, (contact, oldManifold) => {
                    postSolve(contact);
                });
            }
        }
    ...
    CRASH: {
                ...
                component._world = {
                    ...
                    step: function (timeElapsedSinceLastStep) {
                        ...
                        for (let i = 0; i < allColliders.length; i++) {
                            let collider = allColliders[i];
                            entity = collider._entity;
                            let moveX = timeElapsedSinceLastStep / (1000 / ige._fpsRate) * collider.velocity.x;
                            let moveY = timeElapsedSinceLastStep / (1000 / ige._fpsRate) * collider.velocity.y;
                            if (ige.isServer) {
        ...
        contactListener: function (self, beginContactCallback) {
            self.crash.onCollision((a, b, res, cancel) => {
                let entityA = ige.$(a.igeId);
                let entityB = ige.$(b.igeId);
    ...
    BOX2DWEB: {
        ...
        contactListener: function (self, beginContactCallback, endContactCallback, preSolve, postSolve) {
            let contactListener = new self.b2ContactListener();
        ...
        createBody: function (self, entity, body, isLossTolerant) {
            ...
            let tempDef = new self.b2BodyDef();
            let param;
            let tempBod;
            let fixtureDef;
            let tempFixture;
        ...
        init: function (component) {
            ...
            */
            ...
            component.b2Contact.prototype.igeEntityA = function () {
                let ent = this.m_fixtureA.m_body._entity;
            ...
            component.b2Contact.prototype.igeEntityB = function () {
                let ent = this.m_fixtureB.m_body._entity;
                                        ...
                                        switch (fixtureDef.shape.type) {
                                            case `circle`:
                                                ...
                                                */
                                                ...
                                                if (fixtureDef.shape.data && typeof (fixtureDef.shape.data.radius) !== `undefined`) {
                                                    // tempShape.SetRadius(fixtureDef.shape.data.radius / self._scaleRatio);
                                                    let p = new self.b2Vec2(finalX / self._scaleRatio, finalY / self._scaleRatio);
                                                    let r = fixtureDef.shape.data.radius / self._scaleRatio;
                                                    tempShape.Set(p, r);
                                                } else {
    ...
    NATIVE: {
        ...
        createBody: function (self, entity, body) {
            ...
            let tempDef = new self.b2BodyDef(entity._rotate.z);
            let param;
            let tempBod;
            let fixtureDef;
            let tempFixture;
    ...
    BOX2D: {
        ...
        contactListener: function (self, beginContactCallback, endContactCallback, preSolve, postSolve) {
            let contactListener = new self.JSContactListener();
            if (beginContactCallback !== undefined) {
                contactListener.BeginContact = beginContactCallback;
            }
