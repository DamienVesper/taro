...
var IgeEntityBox2d = IgeEntity.extend({
    ...
    updateBody: function (defaultData, isLossTolerant) {
        ...
        let collidesWith = body.collidesWith || {};
        ...
        if (defaultData) {
            ...
            if (defaultData.translate) {
                let x = defaultData.translate.x;
                let y = defaultData.translate.y;
    ...
    destroyBody: function () {
        ...
        if (this.jointsAttached) {
            for (let entityId in this.jointsAttached) {
    ...
    detachEntity: function (entityId) {
        let attachedEntity = ige.$(entityId);
    ...
    applyForceLT: function (x, y) {
        ...
        try {
            if (!isNaN(x) && !isNaN(y) && isFinite(x) && isFinite(y)) {
                ...
                //     y *= 1.2737
                // }
                let thrustVector = new ige.physics.b2Vec2(x, y);
    ...
    _setupContactListeners: function () {
        ...
        ige.physics.contactListener(
            ...
            (contact) => {
                ...
                let arr = self._collisionStartListeners;
            ...
            (contact) => {
                // Loop the collision listeners and check for a match
                let arr = self._collisionEndListeners;
    ...
    _scaleTexture: function () {
        ...
        let body = {};
        if (!currentState) {
            let defaultStateId = self.getDefaultStateId();
        ...
        let newWidth = (body && body.width || 1) * (self._stats.scale);
        let newHeight = (body && body.height || 1) * (self._stats.scale);
    ...
    _scaleBox2dBody: function (scale) {
        ...
        let shapeType = body.fixtures[0].shape && body.fixtures[0].shape.type || `rectangle`;
        let shapeData = {};
    ...
    queueAction: function (action) {
        ...
        if (this._actionQueue && (this._actionQueue.length < 10 || action.type == `destroy` || action.type == `translateTo`)) {
            ...
            let previousAction = this._actionQueue[this._actionQueue.length - 1];
