 ...
 */
let IgePathComponent = IgeEventingClass.extend({
    ...
    set: function (fromX, fromY, fromZ, toX, toY, toZ, findNearest) {
        ...
        // Create a new path
        let path = this._finder.generate(
            this._tileMap,
    ...
    _updateBehaviour: function (ctx) {
        let path = this.path;
        let currentTime = ige._currentTime;
        let progressTime = currentTime - path._startTime;
        ...
        if (path._active && path._totalDistance !== 0 && currentTime >= path._startTime && (progressTime <= path._totalTime || !path._finished)) {
            ...
            let totalDistance = 0;
            let pointArr = path._points;
            let pointCount = pointArr.length;
            let pointIndex;
            let pointFrom;
            let pointTo;
            let newPoint;
    ...
    replacePoints: function (fromIndex, replaceLength, newPoints) {
        let args = [fromIndex, replaceLength].concat(newPoints);
    ...
    _positionAlongVector: function (p1, p2, speed, deltaTime) {
        ...
        let normalisedX = deltaX / magnitude;
        let normalisedY = deltaY / magnitude;
