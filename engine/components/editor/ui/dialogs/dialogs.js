let UiDialogs = IgeEventingClass.extend({
    classId: `UiDialogs`,
        ...
        ige.editor.renderTemplate(
            ...
            (err, dialogElem) => {
                if (!err) {
                    ...
                    let underlay = $(`<div class="editorElem toggleHide shown dialogUnderlay" data-for="${dialogData.id}"></div>`)
    ...
    confirm: function (dialogOptions) {
        ...
        dialogOptions.callback = function (err, dialogElem) {
            if (!err) {
                ...
                let buttons = dialogElem.find(`.actionButtons`);
                buttons.find(`.negative`).on(`click`, () => {
                ...
                buttons.find(`.positive`).on(`click`, () => {
