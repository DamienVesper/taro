let UiPanels = IgeEventingClass.extend({
    classId: `UiPanels`,
                    ...
                    props: {
                        _translate: {
                            ...
                            afterRender: function (obj, propItem) {
                                let selector = $(`#igeEditorProperty_${propItem.id}`);
                    ...
                    props: {
                        _texture: {
                            ...
                            beforeRender: function (obj, propItem) {
                                let textureArr = ige._textureStore;
                                let textureIndex;
                                let tex;
                            ...
                            // Enable any listeners and logic to take action when the user interacts with the panel
                            afterRender: function (obj, propItem) {
                                let panel = $(`#igeEditorProperty_${propItem.id}`);

                                panel.find(`.setTexture`).on(`change`, function () {
                                    let textureId = $(this).val();
                                    ...
                                    let updateMethod = $(`#igeEditorProperty__cell`).data(`igePanelUpdate`);
                                    if (updateMethod) {
                                        updateMethod();
                                    } else {
                                ...
                                panel.find(`.dimensionsFromTexture`).on(`click`, () => {
                                    if (obj._texture) {
                                        obj.dimensionsFromTexture();
                                    }
                                });

                                panel.find(`.dimensionsFromCell`).on(`click`, () => {
                                    if (obj._texture) {
                        ...
                        _cell: {
                            ...
                            afterRender: function (obj, propItem) {
                                ...
                                panel.find(`.image`).on(`click`, function () {
                                    ...
                                    let cellId = parseInt($(this).addClass(`active`).attr(`data-cell-index`));
    ...
    showPanelByInstance: function (obj) {
        if (obj) {
            let classArr = ige.getClassDerivedList(obj);
            let classIndex;
            let className;
                ...
                for (propName in groupProps) {
                    if (groupProps.hasOwnProperty(propName)) {
                        ...
                        (function (groupData) {
                            ...
                            ige.editor.template(`${igeRoot}components/editor/ui/panels/templates/group.html`, (err, template) => {
                                if (!err) {
                                    let groupSelector = $($.parseHTML(template.render(groupData)));
                                    let propName;
                                    let propItem;
                                    let properties = groupData.props;
                                            ...
                                            (function (propData) {
                                                ...
                                                if (propData.obj && (propData.alwaysShow || propData.obj[propData.id])) {
                                                    ...
                                                    if (propertyTemplateUrl) {
                                                        ...
                                                        ige.editor.template(propertyTemplateUrl, (err, template) => {
                                                            if (!err) {
                                                                let propSelector,
                                                                    updateMethod;
                                                                ...
                                                                updateMethod = function () {
                                                                    ...
                                                                    let existingSelector = $(`#igeEditorProperty_${propData.id}`);
                                                                    let existingData = existingSelector.data(`igePanelUpdate`);
