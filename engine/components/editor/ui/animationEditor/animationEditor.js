let UiAnimationEditor = IgeEventingClass.extend({
    ...
    setupListeners: function (dialogElem) {
        ...
        self._framesCanvas.on(`mousemove`, (e) => {
        ...
        self._framesCanvas.on(`mouseout`, (e) => {
            delete self._framesHighlightCell;
        });

        // If canvas is clicked, clear the cell
        self._framesCanvas.on(`click`, (e) => {
            let oe = e.originalEvent;
        ...
        // When moving over the canvas, highlight the cell
        self._cellsCanvas.on(`mousemove`, (e) => {
            let oe = e.originalEvent;
            let cell;
    ...
    cellFromIndex: function (index) {
        index = index - 1;
        let self = this;
        let cellsPerRow = (self._textureImage.width / self._cellWidth);
        let y = Math.floor(index / cellsPerRow);
        let x = index - (y * cellsPerRow);
    ...
    _renderCanvas: function (noGrid) {
        let self = this;
        let framesCtx = self._framesCtx;
        let cellsCtx = self._cellsCtx;
        let outputCtx = self._outputCtx;
        let cell;
        let cellWidth;
        let cellHeight;
        let x; let y; let i;
