let UiTextureEditor = IgeEventingClass.extend({
        ...
        ige.editor.ui.dialogs.create({
            ...
            ready: function (err) {
                if (!err) {
                    ...
                    $(`.control.download`).on(`click`, () => { self.downloadImage(); });
                    $(`.control.split`).on(`click`, () => { self.splitImage(); });
                    $(`.control.clear`).on(`click`, () => { self.clearImage(); });
                    $(`.control.animate`).on(`click`, () => { self.toAnimationEditor(); });
                    $(`.control.help`).on(`click`, () => { self.help(); });
    ...
    setupListeners: function (dndTarget) {
        ...
        self._canvas.on(`mouseout`, (e) => {
            delete self._highlightCell;
        });

        // If canvas is clicked, clear the cell
        self._canvas.on(`click`, (e) => {
            let oe = e.originalEvent;
            let cell = self.cellFromXY(oe);
                ...
                if (dataTransfer.files.length > 1) {
                    ...
                    ige.editor.ui.dialogs.input({
                        ...
                        positive: function () {
                            ...
                            let columns = $(`#multiFileInput`).find(`select`).val();
                            let noBreak = true;
                            let i = 0;
                            let x = 0; let y;
    ...
    setupCanvas: function () {
        ...
        setInterval(() => { self._renderCanvas(); }, 1000 / 60);
    },
    ...
    downloadImage: function () {
        let self = this;
        let drawnArea;
        let form = $(`#textureEditorDialog`).find(`form`);
        let imageDataElem = form.find(`#formImageData`);
        let newCanvas;
    ...
    splitImage: function () {
        let self = this;
        let cols = self._cellCols;
        let rows = self._cellRows;
        ...
        ige.editor.ui.dialogs.input({
            ...
            ready: function () {
                let colsElem = this.find(`#cols`);
                let rowsElem = this.find(`#rows`);

                colsElem.on(`change`, () => {
                    let cols = colsElem.val();
                    cols = cols || 1;
                    self.cellCols(cols);
                });

                rowsElem.on(`change`, () => {
                    let rows = rowsElem.val();
                    rows = rows || 1;
                    self.cellRows(rows);
                });
            },
            ...
            positive: function () {
                ...
                let cols = this.find(`#cols`).val();
                let rows = this.find(`#rows`).val();
    ...
    resizeBackBuffer: function (newWidth, newHeight) {
        ...
        let newBackBuffer = document.createElement(`canvas`);
        let nbbCtx = newBackBuffer.getContext(`2d`);
    ...
    addImage: function (cell, img) {
        ...
        let self = this;
        let x = cell.x * self._cellWidth;
        let y = cell.y * self._cellHeight;
    ...
    occupyCells: function (x, y, w, h) {
        let self = this;
        let cellX = Math.floor(x / self._cellWidth);
        let cellY = Math.floor(y / self._cellHeight);
        let cellW = Math.floor(w / self._cellWidth);
        let cellH = Math.floor(h / self._cellHeight);
        let j; let k;
    ...
    clearCell: function (cell) {
        let self = this;
        let j; let k; let maxX = 0; let maxY = 0;
    ...
    _renderCanvas: function (noGrid) {
        ...
        if (!noGrid) {
            ...
            if (cellWidth > 0 && cellHeight > 0) {
                ...
                for (let x = 0; x < self._canvas[0].width; x += cellWidth) {
                    ctx.beginPath();
                    ctx.moveTo(x, 0);
                ...
                for (let y = 0; y < self._canvas[0].height; y += cellHeight) {
                    ctx.beginPath();
                    ctx.moveTo(0, y);
