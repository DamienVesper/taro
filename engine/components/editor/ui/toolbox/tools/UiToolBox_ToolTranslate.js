let UiToolBox_ToolTranslate = IgeEventingClass.extend({
        ...
        if (val !== undefined) {
            if (ige.editor._selectedObject) {
                ...
                if (val) {
                    ...
                    this._mouseUpHandle = ige.editor.on(`mouseUp`, (event) => {
                        self._mouseUp(event);
                    });

                    this._mouseDownHandle = ige.editor.on(`mouseDown`, (event) => {
                        self._mouseDown(event);
                    });

                    this._mouseMoveHandle = ige.editor.on(`mouseMove`, (event) => {
    ...
    _mouseDown: function (event) {
        if (!this._opStarted) {
            ...
            let mx = (event.igeX - ige._bounds2d.x2);
            let my = (event.igeY - ige._bounds2d.y2);
            let curMousePos = new IgePoint3d(mx, my, 0);
    ...
    _mouseMove: function (event) {
        if (this._enabled && this._targetEntity) {
            ...
            if (this._opStartMouse) {
                let mx = (event.igeX - ige._bounds2d.x2);
                let my = (event.igeY - ige._bounds2d.y2);
                let curMousePos = { x: mx, y: my };
                let panCords = {
                    x: this._opStartMouse.x - curMousePos.x,
                    y: this._opStartMouse.y - curMousePos.y
                }; let distX = Math.abs(panCords.x); let distY = Math.abs(panCords.y);
                let panFinalX = this._opStartTranslate.x - (panCords.x / ige._currentViewport.camera._scale.x);
                let panFinalY = this._opStartTranslate.y - (panCords.y / ige._currentViewport.camera._scale.y);
