 ...
 */
let IgeEditorComponent = IgeEventingClass.extend({
            ...
            $.ajax(url, {
                ...
                complete: function (xhr, status) {
                    if (status === `success`) {
                        ...
                        let template = jsviews.templates(xhr.responseText);
    ...
    createObject: function (classId, select) {
        if (this._selectedObject) {
            let newObj = ige.newClassInstance(classId);
            ...
            if (this.objectDefault[classId]) {
                for (let i in this.objectDefault[classId]) {
    ...
    addToSgTree: function (item) {
        ...
        mouseUp = function (event) {
            ...
            let elems = document.getElementsByClassName(`sgItem selected`);
    ...
    toggleShowEditor: function () {
        ...
        if (this._showSgTree) {
            ...
            let consoleHolderElem = document.createElement(`div`);
            let consoleElem = document.createElement(`input`);
            let classChainElem = document.createElement(`div`);
            ...
            let editorModeTranslate = document.createElement(`input`);
            let editorModeRotate = document.createElement(`input`);
            let editorModeScale = document.createElement(`input`);
            let editorStatus = document.createElement(`span`);
