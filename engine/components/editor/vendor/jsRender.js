...
(function (global, jQuery, undefined) {
    ...
    let versionNumber = `v1.0.0-beta`;

    let $; let jsvStoreName; let rTag; let rTmplString; // nodeJsModule,

    // TODO	tmplFnsCache = {},
    let delimOpenChar0 = `{`; let delimOpenChar1 = `{`; let delimCloseChar0 = `}`; let delimCloseChar1 = `}`; let linkChar = `^`;

    let rPath = /^(!*?)(?:null|true|false|\d[\d.]*|([\w$]+|\.|~([\w$]+)|#(view|([\w$]+))?)([\w$.^]*?)(?:[.[^]([\w$]+)\]?)?)$/g;
    //                                     none   object     helper    view  viewProperty pathTokens      leafToken

    let rParams = /(\()(?=\s*\()|(?:([([])\s*)?(?:(\^?)(!*?[#~]?[\w$.^]+)?\s*((\+\+|--)|\+|-|&&|\|\||===|!==|==|!=|<=|>=|[<>%*:?\/]|(=))\s*|(!*?[#~]?[\w$.^]+)([([])?)|(,\s*)|(\(?)\\?(?:(')|("))|(?:\s*(([)\]])(?=\s*\.|\s*\^|\s*$)|[)\]])([([]?))|(\s+)/g;
    //          lftPrn0        lftPrn        bound            path    operator err                                                eq             path2       prn    comma   lftPrn2   apos quot      rtPrn rtPrnDot                        prn2      space
    // (left paren? followed by (path? followed by operator) or (path followed by left paren?)) or comma or apos or quot or right paren or space

    let rNewLine = /[ \t]*(\r\n|\n|\r)/g;
    let rUnescapeQuotes = /\\(['"])/g;
    let rEscapeQuotes = /['"\\]/g; // Escape quotes and \ character
    let rBuildHash = /\x08(~)?([^\x08]+)\x08/g;
    let rTestElseIf = /^if\s/;
    let rFirstElem = /<(\w+)[>\s]/;
    let rAttrEncode = /[\x00`><"'&]/g; // Includes > encoding since rConvertMarkers in JsViews does not skip > characters in attribute strings
    let rHtmlEncode = rAttrEncode;
    let autoTmplName = 0;
    let viewId = 0;
    let charEntities = {
        "&": `&amp;`,
    ...
    function getView (inner, type) { // view.get(inner, type)
        ...
        let views; let i; let l; let found;
        let view = this;
        let root = !type || type === `root`;
    ...
    function getHelper (helper) {
        ...
        let wrapped;
        let view = this;
        let ctx = view.linkCtx;
        let res = (view.ctx || {})[helper];
    ...
    function convertVal (converter, view, tagCtx) {
        ...
        let tmplConverter; let tag; let value;
        let boundTagCtx = +tagCtx === tagCtx && tagCtx; // if tagCtx is an integer, then it is the key for the boundTagCtx (compiled function to return the tagCtx)
        let linkCtx = view.linkCtx; // For data-link="{cvt:...}"...
    ...
    function renderTag (tagName, parentView, tmpl, tagCtxs, isRefresh) {
        ...
        let render; let tag; let tags; let attr; let parentTag; let i; let l; let itemRet; let tagCtx; let tagCtxCtx; let content; let boundTagFn; let tagDef; let callInit;
        let ret = ``;
        let boundTagKey = +tagCtxs === tagCtxs && tagCtxs; // if tagCtxs is an integer, then it is the boundTagKey
        let linkCtx = parentView.linkCtx || 0;
        let ctx = parentView.ctx;
        let parentTmpl = tmpl || parentView.tmpl;
        let parentView_ = parentView._;
    ...
    function View (context, type, parentView, data, template, key, contentTmpl, onRender) {
        ...
        let views; let parentView_; let tag;
    ...
    function compileChildResources (parentTmpl) {
        let storeName, resources, resourceName, settings, compile;
    ...
    function TmplObject (markup, options) {
        ...
        let htmlTag;
        let wrapMap = $viewsSettings.wrapMap || {};
        let tmpl = $extend(
            {
                markup: markup,
                tmpls: [],
    ...
    function registerStore (storeName, storeSettings) {
        function theStore (name, item, parentTmpl) {
            ...
            let onStore, compile, itemName, thisStore;
    ...
    function renderContent (data, context, parentView, key, isLayout, onRender) {
        ...
        let i; let l; let dataItem; let newView; let childView; let itemResult; let swapContent; let tagCtx; let contentTmpl; let tag_; let outerOnRender; let tmplName; let tmpl;
        let self = this;
        let allowDataLink = !self.attr || self.attr === `html`;
        let result = ``;
    ...
    function tmplFn (markup, tmpl, isLinkExpr, convertBack) {
        ...
        function parseTag (all, bind, tagName, converter, colon, html, comment, codeTag, params, slash, closeBlock, index) {
            ...
            let noError; let current0;
            let pathBindings = bind && [];
            let code = ``;
            let hash = ``;
            let passedCtx = ``;
            // Block tag if not self-closing and not {{:}} or {{>}} (special case) and not a data-link expression
            let block = !slash && !colon && !comment;
            ...
            if (codeTag) {
                ...
                if (params) {
                    ...
                    code = parseParams(params, pathBindings, tmpl)
                        .replace(rBuildHash, (all, isCtx, keyValue) => {
                            if (isCtx) {
                                passedCtx += `${keyValue},`;
                            } else {
                                hash += `${keyValue},`;
    ...
    function buildCode (ast, tmpl, isLinkExpr) {
        ...
        // Used for compiling templates, and also by JsViews to build functions for data link expressions
        let i; let node; let tagName; let converter; let params; let hash; let hasTag; let hasEncoder; let getsVal; let hasCnvt; let useCnvt; let tmplBindings; let pathBindings;
        let nestedTmpls; let tmplName; let nestedTmpl; let tagAndElses; let content; let markup; let nextIsElse; let oldCode; let isElse; let isGetVal; let prm; let tagCtxFn;
        let tmplBindingKey = 0;
        let code = ``;
        let noError = ``;
        let tmplOptions = {};
        let l = ast.length;
    ...
    function parseParams (params, bindings, tmpl) {
        ...
        let named; let bindto; let boundName; // list,
    ...
    $tags({
        ...
        if: {
            render: function (val) {
                ...
                // Otherwise return ""
                let self = this;
                let ret = (self.rendering.done || !val && (arguments.length || !self.tagCtx.index))
                    ? ``
    ...
    function renderForBlock (val) {
        ...
        // We will use the tag.rendering object for carrying rendering state across the calls.
        let self = this;
        let tagCtx = self.tagCtx;
        let noArg = !arguments.length;
        let result = ``;
        let done = noArg || 0;
