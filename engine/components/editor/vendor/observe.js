...
(function (global) {
    ...
    let PROP_ADD_TYPE = `add`;
    let PROP_UPDATE_TYPE = `update`;
    let PROP_RECONFIGURE_TYPE = `reconfigure`;
    let PROP_DELETE_TYPE = `delete`;
    let ARRAY_SPLICE_TYPE = `splice`;
    ...
    let hasObserve = detectObjectObserve();
    ...
    let numberIsNaN = global.Number.isNaN || function isNaN (value) {
    ...
    let identStart = `[\$_a-zA-Z]`;
    let identPart = `[\$_a-zA-Z0-9]`;
    let ident = `${identStart}+${identPart}*`;
    let elementIndex = `(?:[0-9]|[1-9]+[0-9]+)`;
    let identOrElementIndex = `(?:${ident}|${elementIndex})`;
    let path = `(?:${identOrElementIndex})(?:\\s*\\.\\s*${identOrElementIndex})*`;
    ...
    let MAX_DIRTY_CHECK_CYCLES = 1000;
    ...
    function copyObject (object, opt_copy) {
        let copy = opt_copy || (Array.isArray(object) ? [] : {});
        for (let prop in object) {
    ...
    let collectObservers = !hasObserve || global.forceCollectObservers;
    let allObservers;
    ...
    let runningMicrotaskCheckpoint = false;

    let hasDebugForceFullDelivery = typeof Object.deliverAllChangeRecords === `function`;
    ...
    global.Platform.performMicrotaskCheckpoint = function () {
        ...
        do {
            cycles++;
            let toCheck = allObservers;
            allObservers = [];
            results.anyChanged = false;

            for (let i = 0; i < toCheck.length; i++) {
                let observer = toCheck[i];
                if (observer.closed_)
                    continue;
    ...
    ArrayObserver.applySplices = function (previous, current, splices) {
        splices.forEach((splice) => {
            let spliceArgs = [splice.index, splice.removed.length];
    ...
    ObservedSet.prototype = {
        ...
        cleanup: function () {
            let i = 0; let j = 0;
            let isObserved = this.isObserved;
    ...
    CompoundPathObserver.prototype = createObject({
        ...
        addPath: function (object, path) {
            ...
            var path = path instanceof Path ? path : getPath(path);
            let value = path.getValueFrom(object);
        ...
        getValues_: function () {
            ...
            for (let i = 0; i < this.observed_.length; i = i + 2) {
                let path = this.observed_[i + 1];
                let object = this.observed_[i];
                let value = path.getValueFrom(object, this.observedSet_);
                let oldValue = this.values_[i / 2];
                if (!areSameValue(value, oldValue)) {
                    if (!anyChanged && !this.transformFn_) {
                        this.oldValues_ = this.oldValues_ || [];
                        this.changeFlags_ = this.changeFlags_ || [];
                        for (let j = 0; j < this.values_.length; j++) {
        ...
        check_: function () {
            ...
            if (this.transformFn_) {
                ...
                this.reportArgs = [
                    this.values_, this.oldValues_, this.changeFlags_,
                    this.observed_
                ];
            }
    ...
    function notifyFunction (object, name) {
        ...
        let notifier = Object.getNotifier(object);
    ...
    // TODO(rafaelw): Validate errors
        path = getPath(path);
        let notify = notifyFunction(target, name);
        ...
        return {
            close: function () {
                let oldValue = path.getValueFrom(object);
                if (notify)
                    observer.deliver();
                observer.close();
                Object.defineProperty(target, name, {
    ...
    function projectArraySplices (array, changeRecords) {
        ...
        createInitialSplices(array, changeRecords).forEach((splice) => {
