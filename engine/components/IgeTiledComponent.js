...
var IgeTiledComponent = IgeClass.extend({
    ...
    _processData: function (data, callback) {
        ...
        let mapClass = ige.isServer === true ? IgeTileMap2d : IgeTextureMap;
        let mapWidth = data.width;
        let mapHeight = data.height;
        let layerArray = data.layers;
        ...
        let maps = [];
        let layersById = {};
        let tileSetArray = data.tilesets;
        let tileSetCount = tileSetArray ? tileSetArray.length : 0;
        let tileSetItem;
        let tileSetsTotal = tileSetCount;
        let tileSetsLoaded = 0;
        let textureCellLookup = [];
        let currentTexture;
        let currentCell;
        let onLoadFunc;
        let image;
        let textures = [];
        let allTexturesLoadedFunc;
        let i; let k; let x; let y; let z;
        let ent;
        ...
        if (ige.isClient) {
            onLoadFunc = function (textures, tileSetCount, tileSetItem) {
                return function () {
                    ...
                    let imageUrl = tileSetItem.image;
                    let scaleFactor = ige.scaleMapDetails.scaleFactor;
                    ...
                    let cs = new IgeCellSheet(imageUrl, this.width * scaleFactor.x / tileSetItem.tilewidth, this.height * scaleFactor.y / tileSetItem.tileheight, ige.scaleMapDetails.shouldScaleTilesheet)
                        .id(tileSetItem.name)
                        .on(`loaded`, function () {
