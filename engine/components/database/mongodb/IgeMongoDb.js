let IgeMongoDb = {
    ...
    connect: function (callback) {
        ...
        let mongoServer = new this._mongo.Server(
            this._host,
            parseInt(this._port),
            {}
        ); let self = this;
        ...
        this.client.open((err, db) => {
            // If we have a username then authenticate!
            if (self._username) {
                self.client.authenticate(self._username, self._password, (err) => {
                    if (err) {
    ...
    insert: function (coll, json, callback) {
        ...
        this.client.collection(coll, function (err, tempCollection) {
            if (!err) {
                ...
                // Got the collection
                tempCollection.insert(json, (err, docs) => {
                    let i;
    ...
    remove: function (coll, json, callback) {
        ...
        this.client.collection(coll, (err, tempCollection) => {
            if (!err) {
                self.collectionIdToId(coll, json);

                // Got the collection (or err)
                tempCollection.remove(json, { safe: true, single: false }, (err, tempCollection) => {
    ...
    findAll: function (coll, json, callback) {
        ...
        this.client.collection(coll, (err, tempCollection) => {
            if (!err) {
                // Got the collection (or err)
                tempCollection.find(json, (err, tempCursor) => {
                    // Got the result cursor (or err)
                    tempCursor.toArray((err, results) => {
                        let i;
    ...
    update: function (coll, searchJson, updateJson, callback, options) {
        ...
        this.client.collection(coll, (err, tempCollection) => {
            if (!err) {
                ...
                // Got the collection (or err)
                tempCollection.update(searchJson, finalUpdateJson, options, (err, results) => {
                    // Got the result cursor (or err)
                    // Callback the results
                    if (typeof (callback) === `function`) {
                        callback.apply(self, [err, results]);
    ...
    overwrite: function (coll, searchJson, overwriteJson, callback, options) {
        ...
        this.client.collection(coll, (err, tempCollection) => {
            if (!err) {
                ...
                // Got the collection (or err)
                tempCollection.update(searchJson, overwriteJson, options, (err, tempCursor) => {
                    // Got the result cursor (or err)
                    if (!err) {
                        tempCursor.toArray((err, results) => {
                            let i;
