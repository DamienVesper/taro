...
let IgeChatServer = {
    ...
    createRoom: function (roomName, options, roomId) {
        let self = ige.chat;
        let newRoomId = roomId || ige.newIdHex();
    ...
    sendToRoom: function (roomId, message, to, from) {
        ...
        let self = ige.chat;
        let player = ige.game.getPlayerByClientId(from);
        let gameData = ige.game.data && ige.game.data.defaultData;
    ...
    _onMessageFromClient: function (msg, clientId) {
        ...
        if (!self.emit(`messageFromClient`, [msg, clientId])) {
            ...
            if (player) {
                let playerName = player._stats && self.xssFilters.inHTMLData(player._stats.name);
