...
let IgeChatClient = {
    ...
    _onMessageFromServer: function (data) {
        let self = ige.chat;
        let player = ige.game.getPlayerByClientId(data.from);
        ...
        let isChatHidden = $(`#chat-box`).hasClass(`d-none`);
        ...
        if (!self.emit(`messageFromServer`, [data])) {
            ...
            if (!isChatHidden) {
                ...
                if (selectedUnit && selectedUnit.gluedEntities) {
                    ...
                    for (let i = 0; i < selectedUnit.gluedEntities.length; i++) {
                        let gluedEntity = selectedUnit.gluedEntities[i];
                        if (gluedEntity.type === `chatBubble`) {
                            let igeEntity = ige.$(gluedEntity.id);
