let IgeEntityManager = IgeEventingClass.extend({
    ...
    _updateOrphans: function () {
        ...
        let viewportArr = ige._children;
        let vpCount = viewportArr.length;
        let item;
        let itemAabb;
        let vpIndex;
        let inVisibleArea;
        ...
        while (arrCount--) {
            ...
            if (item._managed) {
                if (item.aabb) {
                    ...
                    let isIntersecting = ige.client.vp1.viewArea().intersects(itemAabb);
    ...
    exceptions: function (item, viewArea) {
        ...
        let self = this;
        let parentAABB = item.parent() && item.parent().aabb();

        if (item._category == `item` && item.getOwnerUnit()) {
            parentAABB = item.getOwnerUnit().aabb();
        }

        let checkTranslateInView = function () {
            let translate = {
                x: item._translate.x,
                y: item._translate.y,
        ...
        let checkItemIsBeingUsed = function () {
            if (item._category === `item`) {
                ...
                let itemIsBeingUsed = item._stats.itemIds.find((itemId) => {
                    if (itemId) {
                        let entity = ige.$(itemId);
    ...
    _updateChildren: function () {
        let orphans = this._entity._orphans;
        let viewportArr = ige._children;
        let vpCount = viewportArr.length;
        let item;
        let itemAabb;
