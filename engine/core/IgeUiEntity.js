 ...
 * won't do the job.
 */
let IgeUiEntity = IgeEntity.extend([
    ...
    _renderBorder: function (ctx) {
        let rad;
        let geom = this._bounds2d;
        let left = (-(geom.x2) | 0) + 0.5;
        let top = (-(geom.y2) | 0) + 0.5;
        let width = geom.x - 1;
        let height = geom.y - 1;
    ...
    cell: function (val) {
        let ret = IgeEntity.prototype.cell.call(this, val);
    ...
    mount: function (obj) {
        let ret = IgeEntity.prototype.mount.call(this, obj);
    ...
    tick: function (ctx, dontTransform) {
        ...
        if (!this._hidden && this._inView && (!this._parent || (this._parent._inView)) && !this._streamJustCreated) {
            ...
            if (this._overflow === `hidden`) {
                ...
                // of this one
                let geom = this._bounds2d;
                let left = -(geom.x / 2) + this._paddingLeft | 0;
                let top = -(geom.y / 2) + (this._paddingTop) | 0;
                let width = geom.x + this._paddingRight;
                let height = geom.y + this._paddingBottom;
