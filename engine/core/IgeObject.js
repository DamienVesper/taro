 ...
 */
let IgeObject = IgeEventingClass.extend({
    classId: `IgeObject`,
    ...
    $: function (id) {
        ...
        if (obj._parent === this) {
            ...
            let ancestor = obj.parent(this.id());
    ...
    $$: function (categoryName) {
        let objArr = ige.$$(categoryName);
        let arrCount = objArr.length;
        let obj;
        let finalArr = [];
        let thisId = this.id();
    ...
    unMount: function () {
        ...
        if (this._pixiContainer) {
            if (this._pixiContainer.parent && self.entityId) {
                if (this._pixiTexture.parent.children) {
                    var index = this._pixiContainer.parent.children.findIndex((child) => { return child.entityId == self.entityId; });
                    if (index > -1) {
        ...
        if (this._parent) {
            let childArr = this._parent._children;
    ...
    clone: function (options) {
        ...
        let newObject = eval(this.stringify(options));
                    ...
                    if (this._mountMode === 1) {
                        ...
                        if (this._depthSortMode === 0) { // Slowest, uses 3d bounds
                            ...
                            this._children.sort((a, b) => {
                                let layerIndex = b._layer - a._layer;
    ...
    viewCheckChildren: function () {
        if (ige._currentViewport) {
            let arr = this._children;
            let arrCount = arr.length;
            let vpViewArea = ige._currentViewport.viewArea();
            let item;
    ...
    tick: function (ctx, renderChildBelowUnit) {
        ...
        if (this._alive) {
            let arr = this._children;
            let parentLayer = parseInt(this.layer());
            let parentDepth = parseInt(this.depth());
            let arrCount;
            let ts; let td;
    ...
    loadGraph: function (obj) {
        if (obj.igeClass && obj.data) {
            ...
            let classInstance = ige.newClassInstance(obj.igeClass);
            let newId;
            let childArr;
            let childIndex;
            let parentId;
