...
var IgeEntity = IgeObject.extend({
    ...
    init: function (defaultData = {}) {
        ...
        let translateX = defaultData.translate && defaultData.translate.x ? defaultData.translate.x : 0;
        let translateY = defaultData.translate && defaultData.translate.y ? defaultData.translate.y : 0;
        let rotate = defaultData.rotate || 0;
    ...
    show: function () {
        ...
        if (ige.isServer) {
            ...
            self._hidden = false;
            let pixiEntity = self._pixiText || self._pixiTexture;
            if (pixiEntity) {
    ...
    setState: function (stateId, defaultData) {
        ...
        let newState = (self._stats.states && self._stats.states[stateId]) || {};
        ...
        if (ige.isServer) {
            ...
            if (newState.sound) {
                for (let soundId in newState.sound) {
                    let sound = newState.sound[soundId];
    ...
    applyAnimationById: function (animationId) {
        ...
        if (animation && cellSheet) {
            ...
            let cellSheetAnimId;
            let url = cellSheet.url;
            let rows = cellSheet.rowCount;
            let columns = cellSheet.columnCount;
            ...
            let fps = animation.framesPerSecond || 15;
            let loopCount = animation.loopCount - 1; // Subtract 1 for Jaeyun convention on front end
    ...
    applyAnimationForState: function (stateId) {
        ...
        let state = ige.game.data.states[stateId];
        ...
        if (state) {
            let animationId = state.animation;
            if (animationId) {
    ...
    updateTexture: function () {
        let stateId = this._stats.stateId;
    ...
    cache: function (val) {
        if (val !== undefined) {
            ...
            if (val) {
                ...
                // Set smoothing mode
                let smoothing = this._cacheSmoothing !== undefined ? this._cacheSmoothing : ige._globalSmoothing;
                if (!smoothing) {
    ...
    createPixiTexture: function (defaultSprite = 0, defaultData) {
        let texture = new IgePixiTexture(this._stats.cellSheet.url, this._stats.cellSheet.columnCount, this._stats.cellSheet.rowCount, this);
    ...
    occupyTile: function (x, y, width, height) {
        ...
        if (this._parent && this._parent.IgeTileMap2d) {
            if (x !== undefined && y !== undefined) {
                ...
                let trPoint = new IgePoint3d(this._translate.x - (this._tileWidth / 2 - 0.5) * this._parent._tileWidth, this._translate.y - (this._tileHeight / 2 - 0.5) * this._parent._tileHeight, 0);
                let tilePoint = this._parent.pointToTile(trPoint);
    ...
    overTiles: function () {
        ...
        if (this._parent && this._parent.IgeTileMap2d) {
            let x;
            let y;
            let tileWidth = this._tileWidth || 1;
            let tileHeight = this._tileHeight || 1;
            let tile = this._parent.pointToTile(this._translate);
            let tileArr = [];
    ...
    localBounds3dPolygon: function (recalculate) {
        if (this._bounds3dPolygonDirty || !this._localBounds3dPolygon || recalculate) {
            let geom = this._bounds3d;
            let poly = new IgePoly2d();
            // Bottom face
            let bf2 = Math.toIso(+geom.x2, -geom.y2, -geom.z2);
            let bf3 = Math.toIso(+geom.x2, +geom.y2, -geom.z2);
            let bf4 = Math.toIso(-geom.x2, +geom.y2, -geom.z2);
            // Top face
            let tf1 = Math.toIso(-geom.x2, -geom.y2, geom.z2);
            let tf2 = Math.toIso(+geom.x2, -geom.y2, geom.z2);
            let tf4 = Math.toIso(-geom.x2, +geom.y2, geom.z2);
    ...
    bounds3dPolygon: function (recalculate) {
        if (this._bounds3dPolygonDirty || !this._bounds3dPolygon || recalculate) {
            let poly = this.localBounds3dPolygon(recalculate).clone();
    ...
    mouseInBounds3d: function (recalculate) {
        let poly = this.localBounds3dPolygon(recalculate);
    ...
    _projectionOverlap: function (otherObject) {
        ...
        let otherG3d = otherObject._bounds3d;
        let otherMin = {
    ...
    isBehind: function (otherObject) {
        let thisG3d = this._bounds3d;
        let otherG3d = otherObject._bounds3d;
        let thisTranslate = this._translate.clone();
        let otherTranslate = otherObject._translate.clone();
        ...
        let thisX = thisTranslate.x;
        let thisY = thisTranslate.y;
        let otherX = otherTranslate.x;
        let otherY = otherTranslate.y;
        let thisMin = new IgePoint3d(thisX - thisG3d.x / 2, thisY - thisG3d.y / 2, this._translate.z);
        let thisMax = new IgePoint3d(thisX + thisG3d.x / 2, thisY + thisG3d.y / 2, this._translate.z + thisG3d.z);
        let otherMin = new IgePoint3d(otherX - otherG3d.x / 2, otherY - otherG3d.y / 2, otherObject._translate.z);
        let otherMax = new IgePoint3d(otherX + otherG3d.x / 2, otherY + otherG3d.y / 2, otherObject._translate.z + otherG3d.z);
    ...
    playEffect: function (type) {
        if (this._stats && this._stats.effects && this._stats.effects[type]) {
            let effect = this._stats.effects[type];
            ...
            if (ige.isServer) {
                ...
                if (effect.animation == undefined || effect.animation == `none`) {
                    let currentState = this._stats.states[this._stats.stateId];
                ...
                if (effect.projectileType) {
                    let projectile = ige.game.getAsset(`projectileTypes`, effect.projectileType);
                    if (projectile) {
                ...
                if (effect.sound) {
                    for (let soundKey in effect.sound) {
                        if (!isAlreadyPlaying) {
                ...
                let angle = this._rotate.z;
                if (type == `attacked`) {
                    // get angle between attacked unit and attacking unit
                    let attacker = this.lastAttackedBy;
                    if (attacker) {
    ...
    flip: function (flip) {
        ...
        if (ige.isServer) {
            ...
            if (this._stats.flip != flip) {
                ...
                let entity = this._pixiTexture;
                if (entity) {
                    let x = Math.abs(entity.scale.x);
                    let y = Math.abs(entity.scale.y);
                    if (flip == 0) {
    ...
    _renderCache: function (ctx) {
        ctx.save();
        if (this._compositeCache) {
            let aabbC = this._compositeAabbCache;
    ...
    destroy: function (destroyOrphan) {
        ...
        if (this.gluedEntities && this.gluedEntities.length > 0) {
            this.gluedEntities.forEach((glueEntity) => {
                let entity = ige.$(glueEntity.id);
                if (entity && (entity._pixiText || entity._pixiTexture)) {
                    let texture = entity._pixiText || entity._pixiTexture;
                    entity.unMount();
                    entity.destroy();
        ...
        if (ige.isClient) {
            let entityId = this.entityId || this.id();
            if (ige.pixi.trackEntityById[entityId]) {
                ...
                let texture = ige.pixi.trackEntityById[entityId]._pixiTexture || ige.pixi.trackEntityById[entityId]._pixiText || ige.pixi.trackEntityById[entityId];
            ...
            if (this.attributeBars) {
                for (let attributeBarInfo of this.attributeBars) {
                    let pixiBarId = attributeBarInfo.id;
                    let pixiBar = ige.$(pixiBarId);
         ...
         */
        for (let region in ige.regionManager.entitiesInRegion) {
    ...
    clearAllPointers: function () {
        let keysToDelete = [`ability`, `animation`, `attribute`, `attributeBarsContainer`, `fadingTextContainer`, `inventory`, `minimapUnit`, `unitNameLabel`, `unitUi`, `_aabb`, `_bounds2d`, `_bounds3d`, `_rotate`, `_velocity`];
    ...
    transformPixiEntity: function (x, y, z, type) {
        ...
        let entity = this._pixiText || this._pixiContainer;
    ...
    isInVP: function (rect2) {
        let vp = ige.pixi.viewport.getVisibleBounds();
        let rect1 = {
            x1: vp.x,
            y1: vp.y,
            x2: vp.x + vp.width,
    ...
    updateTransform: function () {
        ...
        if (this._mode === 1) {
            if (!isNaN(this._translate.x) && !isNaN(this._translate.y)) {
                // iso translation
                let isoPoint = (this._translateIso = new IgePoint3d(this._translate.x, this._translate.y, this._translate.z + this._bounds3d.z / 2).toIso());
    ...
    getPersistentData: function (type) {
        ...
        let variablesToBeIgnored = [`unit`, `item`, `player`, `playerGroup`, `itemGroup`, `unitGroup`, `projectile`, `region`];
        for (let variablesId in self.variables) {
            let variable = self.variables[variablesId];
        ...
        if (isUnitExists) {
            ...
            unit._stats.itemIds.forEach((itemId) => {
                if (itemId) {
        ...
        if (isUnitExists && type == `unit` && unit._stats.itemIds) {
            let inventoryItems = unit._stats.itemIds.reduce((pv, cv) => {
                if (cv) {
                    let item = ige.$(cv);
                    if (item) {
                        let itemVariable = {};
                        for (let variablesId in item.variables) {
                            let variable = item.variables[variablesId];
                            if (!variablesToBeIgnored.includes(variable.dataType)) {
                                itemVariable[variablesId] = JSON.parse(JSON.stringify(variable));
                            }
                        }
                        let itemStatsToBeSaved = {
    ...
    updateStats: function (itemId, removeAttributes) {
        ...
        let currentType = this._category === `unit` ? `unitTypes` : `playerTypes`;
        let bonusType = this._category === `unit` ? `unitAttribute` : `playerAttribute`;
        let currentEntityTypeId = this._category === `unit` ? `type` : `playerTypeId`;
        let baseEntityStats = ige.game.getAsset(currentType, this._stats[currentEntityTypeId]);
        ...
        if (item && item._stats.bonus && item._stats.bonus.passive && unit && (unit.canUseItem(item._stats) || removeAttributes)) {
            let attributePassiveBonuses = item._stats.bonus.passive[bonusType];

            if (attributePassiveBonuses) {
                for (let attrId in attributePassiveBonuses) {
                    let selectedAttribute = this._stats.attributes[attrId];
                    let bonus = attributePassiveBonuses[attrId];

                    if (selectedAttribute && bonus) {
                        let currentAttributeValue = parseFloat(selectedAttribute.value) || 1;
                        let maxValue = parseFloat(selectedAttribute.max);
    ...
    loadPersistentData: function (persistData) {
        ...
        if (persistData) {
            let attributes = persistData.attributes;
            let variables = persistData.variables;
            let max = {};
            let min = {};
            let regSpeed = {};
            let keysToStream = [`min`, `max`, `regenerateSpeed`, `value`];
            for (let attrKey in attributes) {
                let persistAttribute = attributes[attrKey];
                let playerAttribute = self._stats && self._stats.attributes && self._stats.attributes[attrKey];
                if (playerAttribute) {
                    for (let i = 0; i < keysToStream.length; i++) {
                        let key = keysToStream[i];
                        switch (key) {
            ...
            for (let variableKey in variables) {
                let persistVariable = variables[variableKey];
    ...
    streamUpdateData: function (queuedData) {
        ...
        if (queuedData != undefined) {
            for (let i = 0; i < queuedData.length; i++) {
                let data = queuedData[i];
                for (attrName in data) {
                    let newValue = data[attrName];
                    // console.log(this._category, this.id(), attrName, newValue)
                    switch (attrName) {
                        case `attributes`:
                            // only on client side to prevent circular recursion
                            if (ige.isClient) {
                                let attributesObject = _.cloneDeep(this._stats.attributes);
                                if (attributesObject) {
                                    for (var attributeTypeId in data.attributes) {
                                        let attributeData = attributesObject[attributeTypeId];

                                        if (attributeData) {
                                            let newAttributeValue = data.attributes[attributeTypeId];
                                            let oldAttributeValue = attributeData.value;
                                ...
                                for (var attributeTypeId in data.attributes) {
                                    ...
                                    if (this._stats.attributes && this._stats.attributes[attributeTypeId]) {
                                        let attribute = this._stats.attributes[attributeTypeId];
                                        if (
                    ...
                    if (ige.isServer) {
                        ...
                        let forceStreamKeys = [`anim`, `coin`, `stateId`, `ownerId`, `name`, `slotIndex`, `newItemId`, `quantity`, `spriteOnly`, `setFadingText`, `playerJoinedAgain`, `use`, `hidden`];
    ...
    streamSectionData: function (sectionId, data) {
        switch (sectionId) {
            case `transform`:
                if (ige.isServer) {
                    let x = this._translate.x.toFixed(0);
                    let y = this._translate.y.toFixed(0);
                    let angle = ((this._rotate.z % (2 * Math.PI)) * 1000).toFixed(0);
    ...
    streamCreateData: function (clientId) {
        if (ige.isServer) {
            ...
            let keys = ige.server.keysToRemoveBeforeSend.slice();
    ...
    _streamSync: function () {
        let recipientArr = ige.network.clientIds;
    ...
    streamCreate: function (clientId) {
        ...
        if (this._parent && ige.network.stream._streamClientCreated !== undefined) {
            ...
            let streamCreateData = this.streamCreateData(clientId);
    ...
    interpolateValue: function (startValue, endValue, startTime, currentTime, endTime) {
        ...
        let dataDelta = endTime - startTime;
        let offsetDelta = currentTime - startTime;
    ...
    reconcileValue: function (startValue, endValue, startTime, currentTime, endTime) {
        ...
        let dataDelta = endTime - startTime;
        let offsetDelta = currentTime - startTime;
        let t = Math.abs(offsetDelta / dataDelta);
        let deltaTime = t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t; // easeInOutQuad (acceleration until halfway, then deceleration)
    ...
    _processTransform: function () {
        ...
        if (prevTransform != undefined && nextTransform != undefined &&
            ...
            let startValue = prevKeyFrame[1][2];
            let endValue = nextKeyFrame[1][2];
        ...
        if (this == ige.client.selectedUnit) {
            let ownerPlayer = ige.$(this._stats.ownerId);
            if (ownerPlayer) {
    ...
    getAttributeBarContainer: function () {
        let self = this;
        let attributeContainerId = self._stats && self._stats.attributeBarContainer && self._stats.attributeBarContainer.id;
