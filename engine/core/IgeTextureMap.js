 ...
 */
let IgeTextureMap = IgeTileMap2d.extend({
    ...
    negate: function (entity) {
        if (entity !== undefined) {
            let x; let y;
            let entityMapData = entity.map._mapData;
            let thisMapData = this.map._mapData;
    ...
    tick: function (ctx) {
        ...
        let finalPoint;
        let tileData; let tileEntity = this._newTileEntity(); // TODO: This is wasteful, cache it?
        let sectionX; let sectionY;
        let tempSectionX; let tempSectionY;
        let _ctx;
        let regions; let region; let i;
    ...
    _drawSectionsToCtx: function (ctx) {
        let x; let y; let tileData;
        let sectionRenderX; let sectionRenderY;
        let sectionAbsX; let sectionAbsY;
        let sectionWidth; let sectionHeight;
        let viewArea = ige._currentViewport.viewArea();
    ...
    _renderTile: function (ctx, x, y, tileData, tileEntity, rect, sectionX, sectionY) {
        ...
        let finalX; let finalY; let regions;
        let xm1; let xp1; let ym1; let yp1; let regObj;
        let xAdjust = this._mountMode === 1 ? this._tileWidth / 2 : 0;
        let yAdjust = this._mountMode === 1 ? this._tileHeight / 2 : 0;
        let tx; let ty; let sx; let sy;
        let texture;
