...
let IgeFSM = IgeClass.extend({
    ...
    initialState: function (stateName, data, callback) {
        let newStateObj = this.getState(stateName);
        ...
        if (newStateObj.enter) {
            newStateObj.enter.apply(newStateObj, [
                data, function (enterErr, enterData) {
    ...
    _transitionStates: function (oldStateName, newStateName, data, callback) {
        let self = this;
        let currentStateObj = self.getState(self._currentStateName);
        let newStateObj = self.getState(newStateName);

        if (currentStateObj && newStateObj) {
            if (self._debug) { self.log(`Exiting state: ${self._currentStateName}`); }
            if (currentStateObj.exit) {
                currentStateObj.exit.apply(currentStateObj, [
                    data, function (exitStateErr, exitStateData) {
                        ...
                        if (newStateObj.enter) {
                            newStateObj.enter.apply(newStateObj, [
                                data, function (enterStateErr, enterStateData) {
