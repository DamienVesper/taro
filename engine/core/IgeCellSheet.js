 ...
 * width and height.
 */
let IgeCellSheet = IgeTexture.extend({
    ...
    init: function (url, horizontalCells, verticalCells, shouldScale, spriteCellSheet, entity) {
        ...
        let callbackIfNotLoaded = function (err) {
            if (err && entity) {
                let entityType = entity._category === `item` ? entity._stats.itemTypeId : entity._stats.type;
                let defaultUrl = ige.game.getAsset(`${entity._category}Types`, entityType).cellSheet.url;
	 ...
	 */
    stringify: function () {
        let str = `new ${this.classId()}('${this.url()}', ${this.horizontalCells()}, ${this.verticalCells()})`;
