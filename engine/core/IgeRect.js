...
var IgeRect = IgeClass.extend({
    ...
    combineRect: function (rect) {
        let thisRectMaxX = this.x + this.width;
        let thisRectMaxY = this.y + this.height;
        let thatRectMaxX = rect.x + rect.width;
        let thatRectMaxY = rect.y + rect.height;

        let x = Math.min(this.x, rect.x);
        let y = Math.min(this.y, rect.y);
        let width = Math.max(thisRectMaxX - this.x, thatRectMaxX - this.x);
        let height = Math.max(thisRectMaxY - this.y, thatRectMaxY - this.y);
